...
.PARAMETER Version
    ...
    リリース時はタグをそのまま渡す（例: v0.6.0 → tako-setup-v0.6.0-x64.exe）。
...
Write-Host "== tako Windows 配布物ビルド (version=$Version) =="
...
& $iscc "/DAppVersion=$Version" "/DBinDir=$binDirFull" "/O$OutDir" $issPath
if ($LASTEXITCODE -ne 0) { throw "ISCC に失敗 (exit $LASTEXITCODE)" }

$setupExe = Join-Path $OutDir "tako-setup-$Version-x64.exe"
if (-not (Test-Path -LiteralPath $setupExe)) { throw "インストーラーが生成されていない: $setupExe" }

if (-not $SkipZip) {
    Write-Host '-- ポータブル zip'
    $zipPath = Join-Path $OutDir "tako-$Version-windows-x64.zip"
