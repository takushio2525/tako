...
夜間パッチリリース（自動）。v0.8.13 以降の変更:

- [改善] master system prompt の取り分を tako と利用者で分け、追記を自動分割する (#1477) (#1478)
- [機能追加] 蓋閉じ継続をバッテリー駆動でも opt-in で続けられるようにした (#1473) (#1476)
- [機能追加] 人がやることの添付を、行を押すだけで既存プレビューで開けるようにした (#1472) (#1475)
- [機能追加] スマホのタスク画面で添付をその場で見る・鳴らす (#1472) (#1474)
- [ドキュメント] activeContext を 2026-09-14 の現在状態へ更新 (#1471)
- [修正] worker が起票したユーザータスクの返答を spawn 元の master へ返す (#1466) (#1470)
- [修正] MCP カタログの enum を正本から生成するようにした (#1467) (#1469)
- [ドキュメント] 人待ちの項目は引き継ぎではなく tako todo へ、を handoff の手順書に足した (#1450) (#1468)
- [リファクタ] 右パネルの手書きテキスト入力 2 本を TextField へ寄せた (#1459) (#1465)
- [機能追加] 人がやることをスマホから片付けられるようにした (#1450) (#1464)

## [0.8.13] - 2026-09-14

Nightly patch release (automated). Changes since v0.8.12:
夜間パッチリリース（自動）。v0.8.12 以降の変更:

- [機能追加] 人がやることを右パネルで見て、その場で返せるようにした (#1450) (#1463)
- [修正] 番犬の共有部品が合成 cfg のテストモジュールを本番として走査していた (#1445) (#1462)
- [修正] preview パネルつき AskUserQuestion（2 カラム配置）を検知できるようにした (#1447) (#1461)
- [機能追加] リモートの「権限が足りません」に導線を付け、端末ごとの権限編集を PC へ置いた (#1452) (#1460)
- [機能追加] master のプロファイル運用を自動化し、素の master をその場で専用へ寄せられるようにした (#1453) (#1458)
- [機能追加] スマホのファイル閲覧を Finder 風の全体閲覧へ広げ、ショートカットを足せるようにした (#1451) (#1457)
- [修正] SSH ペインの自動再接続が発火しないケースを塞いだ (#1446) (#1456)
- [機能追加] ユーザー向けタスクの正本と、返答を起票 master へ返す配送 (#1450) (#1455)
- [機能追加] リモート PWA の「+」を 3 択にして、ターミナル / SSH ターミナルも新規に起動できるようにした (#1449) (#1454)
- [修正] 窓の位置・寸法を tako 自身の口で決められるようにした (#1442) (#1448)
- [修正] 深い data dir で IPC が SUN_LEN 超過で立たず黙って縮退するのを直した (#1441) (#1444)
- [改善] worker spawn は別タブへ逃がさず、同じタブでフォントを縮めて桁数を確保する (#1439) (#1443)
- [スタイル] ペイン枠線をルート側オーバーレイ 1 枚へ集約した (#812) (#1440)

## [0.8.12] - 2026-09-13

Nightly patch release (automated). Changes since v0.8.11:
夜間パッチリリース（自動）。v0.8.11 以降の変更:

- [修正] ループバック TCP の偽 X-Forwarded-For を接続元プロセスの検証で塞いだ (#841) (#1437)
- [修正] UI モジュールに残る eprintln! 10 件を通知欄 / 診断へ振り分けた (#1432) (#1436)
- [改善] 裏タブの寸法合わせを毎フレームから key + 間引きへ寄せた (#1426) (#1435)
- [修正] ファイルツリーのスキャン対象が毎回 2 倍になり消えたルート配下を読み続けるのを直した (#1404) (#1434)
- [改善] merge-pr.sh の終了コードを実測で言い切り、ローカル head も自分で消した (#1430) (#1433)
- [修正] 右パネルの tmux 復元と UI の eprintln! 5 か所を通知欄 / 診断へ寄せた (#1422) (#1431)
- [ドキュメント] activeContext を 2026-09-12 の実態へ同期 (#1428)
- [改善] save_layout の変化検出を JSON 直列化の前へ出した (#1425) (#1429)
- [修正] ファイルツリーが 1 ディレクトリ 500 件を超えたぶんを黙って捨てるのを直した (#1402) (#1427)
- [改善] 番犬の走査範囲が途中の #[cfg(test)] で切れるのを直した (#1420) (#1424)
- [修正] 右パネルの tmux window 切替とプレビューの目次 / ビュー切替の失敗を通知欄へ出した (#1417) (#1421)
- [修正] 器なし（tmux 無し / persist OFF）でもチャットビューが立つようにした (#1397) (#1423)
- [改善] セルフテストに残る「固定予算 + CLI の状態読み」16 件を状態待ちへ移して KNOWN_FIXED_CLI_WAITS を空にした (#1375) (#1416)
- [修正] ファイルツリーでディレクトリへのシンボリックリンクがファイル扱いになるのを直した (#1398) (#1419)
- [修正] remote daemon の HTTP 受信を少数ワーカーへ並列化した (#1403) (#1418)
- [修正] tako 自身が開いた SSH ペインを自動検知が見送るのを直した (#1411) (#1415)
- [修正] ファイルツリーのローカル操作の失敗を共有の通知欄へ出した (#1399) (#1414)
- [修正] ~/.ssh/config の Match ブロックの設定が直前の Host へ混入するのを直した (#1400) (#1413)
- [修正] remote stop の stale 経路にも PID の正体確認を通した (#1401) (#1412)

## [0.8.11] - 2026-09-12

Nightly patch release (automated). Changes since v0.8.10:
夜間パッチリリース（自動）。v0.8.10 以降の変更:

- [ドキュメント] 脅威モデルの「別 OS ユーザーの到達は消滅した」を現行の既定へ揃えた (#1406) (#1410)
- [改善] リモート / SSH の手順を新 topic `remote` へ明文化した (#1004) (#1409)
- [修正] 器なしペインの busy を close 確認 / GUI 判定 / agent_running へ届けた (#1367) (#1396)
- [ドキュメント] visible_lines_filled の「行末が全角」の取りこぼしを限界として固定した (#1389) (#1395)
- [リファクタ] 読み手のいない「行に全角が在るか」の旗を落とした (#1388) (#1394)
- [改善] terminal-core のテストの穴 4 件を埋め、罫線つき入力欄の誤判定を直した (#1390) (#1393)
- [修正] IPC 由来のレイアウト変更のあと 1 フレーム強制描画するようにした (#1370) (#1392)
- [修正] NFD の結合文字が画面テキストから落ちるのを 3 経路まとめて直した (#1387) (#1391)
- [修正] PDF のリンク注釈と提案チップにスキーム検査を通した (#1376) (#1386)
- [修正] clippy が単体形と workspace 形で違う lint を見る取りこぼしを塞いだ (#1383) (#1384)
- [ドキュメント] 全選択 Cmd+A の効く先を実態へ直し番犬で拘束した (#1362) (#1382)
- [修正] セルフテスト 3 項目の固定予算を状態待ちへ寄せ、番犬を状態読みへ広げた (#1353 / #1364) (#1381)
- [修正] 実 PTY の fixture を待つテストを状態待ちへ寄せた (#1308) (#1380)
- [修正] 蓋閉じ継続の記録に所有者を持たせ原子書き込み + fail-loud にした (#1373) (#1379)
- [修正] Windows の URL 起動から cmd.exe を外し ShellExecuteW へ寄せた (#1371) (#1378)
- [修正] exe::find が npm シムの裸スクリプトを PATHEXT より先に返さないようにした (#1372) (#1377)
- [修正] 狭い実行ペインで折り返した exit マーカーを拾えるようにした (#651) (#1369)
- [改善] 承認カードの command を「承認を求めている操作」から始める (#633) (#1368)
- [改善] 衝突で CI の run が作られない状態を待たずに名指しで案内する (#1365) (#1366)
- [ドキュメント] PWA e2e の CI 追加時間を 2 run の実測幅へ直した (#1357) (#1363)
- [修正] 器を持たないペインも sleep guard の busy に数えるようにした (#372) (#1360)
- [修正] GUI 経路の close が workers.yaml へ発生源つきで closed を記録するようにした (#775) (#1359)
- [改善] ログイン失効を接続断・上限とは別種として検知する (#757) (#1354)
- [改善] PWA の Playwright e2e を CI へ載せ、実行手順を README とコマンド表に置いた (#1357) (#1361)
- [ドキュメント] コピー行の「選択なしなら Ctrl+C を送信」を実測した実態へ直し、番犬で拘束した (#1349) (#1358)
- [ドキュメント] 承認カード e2e 3 本が #1089 で修正済みであることを実測で確定した (#632) (#1355)
- [ドキュメント] 作業ログを起動時ロードの予算へ戻した (#1312) (#1356)
- [修正] テスト本体が作る使い捨て dir をスコープで消える器へ寄せた (#1312) (#1345)
- [ドキュメント] 作業ログに #1347 を追記し、予算超過を解消した (#1351)
- [改善] 2 秒 tick の全ペイン フルスナップショット 2 本を末尾窓へ寄せた (#1301) (#1346)
- [修正] merge 後のリモートブランチ削除を merge-pr.sh 自身で閉じた (#1347) (#1348)
- [ドキュメント] progress.md に #1316 を追記し、予算超過を解消した (#1316) (#1350)
- [ドキュメント] remote.rs の transport doc を #1038 後の実態へ直した (#1332) (#1341)
- [ドキュメント] docs の MCP ツール数 / CLI コマンド数を実装に合わせ、件数と網羅性を番犬で拘束した (#1316) (#1334)
- [改善] 要件番号の一意性とコードからの参照の実在を番犬で固定した (#1336) (#1338)
- [ドキュメント] docs サイトの取り残し 3 ページを実挙動へ合わせた (#1317 #1322 #1323) (#1335)
- [改善] PR の CI 完了待ちを共通スクリプトへ寄せ、checks 登録前の merge を止めた (#1333) (#1337)
- [修正] WorkerStatusCtx のテスト構築に input_style を足して main の all-targets ビルドを直した (#1343) (#1344)
- [ドキュメント] README のリモート transport と tako --help の説明を実態へ直した (#1318 #1321) (#1331)
- [修正] 実行拒否ゲートを Some(false) に締めて claude で常に開く穴を塞いだ (#1295) (#1329)
- [修正] 入力欄の AI ゴースト提案を「人の下書き」と読まないようにした (#1297) (#1330)
- [ドキュメント] AI 向け規約ファイルの取り残しを実態へ揃えた (#1320 #1324 #1319) (#1327)
- [修正] ゾンビ pid テストの「2 秒以内に返る」を機構の観測値へ替えた (#962) (#1310)
- [改善] AI 自動命名の claude 呼び出しから MCP サーバーを外した (#758) (#1311)
- [修正] [Windows] psmux の conf を tmp → rename で差し替えるようにした (#1314) (#1326)
- [修正] config_io の tmp 名を書き込みごとに分けて同一プロセス内の競合を止めた (#1313) (#1328)
- [修正] ペインへの TERM / COLORTERM 注入を最後の無条件上書きへ寄せた (#946) (#1325)
- [改善] 新規 worktree で PWA の dist が無くても cargo build が通るようにした (#1309) (#1315)
- [修正] send_input の queued 応答が前回の決着済み送達の顛末を名乗らないようにした (#1292) (#1306)
- [修正] シェル統合の状態ファイルの tmp 名を書き込みごとに分けて同一プロセス内の競合を止めた (#638) (#1304)
- [修正] cargo test の pid ごとの data dir を終了時と次回起動時に掃くようにした (#1296) (#1303)
- [ドキュメント] 実時間比較フレークの再現の作法を規約へ（#1208 は #1220 で修正済みと確認） (#1307)
- [修正] trust_auto_accept_e2e の tmux 起動失敗を診断で割れる形にして真因を直した (#1300) (#1305)
- [修正] peer 送達の「送ったかもしれない」を未達扱いにせず自動再送の二重投函を止めた (#1294) (#1302)
- [改善] [Windows] tako tmux cleanup --servers を psmux の器でも使えるようにした (#1282) (#1287)
- [修正] 会話ログの番号つき箇条書きがダイアログの選択肢に混ざらないようにした (#1293) (#1299)
- [修正] no_personal_data 番犬が gitignore 済みの未追跡ファイルで落ちないようにした (#1035) (#1298)
- [改善] codex / agy の背景作業つき入力待ちの判定可否を MATRIX へ確定した (#1277) (#1288)
- [修正] osc7 系テストを状態待ちにして不着の原因が診断で割れるようにした (#1265) (#1290)
- [修正] 単体テストが実 agy / codex を起動して本番ホームへ書かないようにした (#1261) (#1289)

## [0.8.10] - 2026-09-10

Nightly patch release (automated). Changes since v0.8.9:
夜間パッチリリース（自動）。v0.8.9 以降の変更:

- [修正] ~ 始まりのパスを cmd+クリックで開けるようにした (#1283) (#1286)
- [修正] 隔離セルフテストの事前信頼が本番 ~/.claude.json へ書かないようにした (#1253) (#1257)
- [機能追加] 解説動画をスライド主軸へ作り直し、X 向けショートを追加した (#1081 / #1284) (#1285)
- [改善] 本番でも SIGTERM で layout を保存して終了するようにした (#777) (#1256)
- [修正] [Windows] psmux の後始末に期限をつけて器のリークを止めた (#1271) (#1281)
- [修正] ui_text の言語依存テストを言語固定の下で比較させフレークを止めた (#1274) (#1280)
- [修正] 背景シェルが残る worker でも空の入力欄が出ていれば idle と判定するようにした (#1273) (#1279)
- [修正] [Windows] #1199 の検証ブロックを本来の関数へ移して cargo test のコンパイルを直した (#1264) (#1276)
- [改善] 直接ペインのスクロールバック上限を設定可能にした (#818) (#1275)
- [修正] 背景シェルが残る idle ペインへ send_input が無音で届かない問題を直した (#1259) (#1272)
- [修正] [Windows] psmux の永続化 e2e を状態待ちにして間欠失敗を止めた (#1114) (#1270)
- [修正] ssh 自動追加の見送りログを同一プロセスで 1 回だけにした (#1258) (#1269)
- [修正] tmux_backend のマウスレポートテストを状態待ちにしてフレークを止めた (#1252) (#1268)
- [修正] claude の Teach auto mode ダイアログを select として検知して respond できるようにした (#1263) (#1267)
- [ドキュメント] 解説動画の収録開始条件を 3 つに定義した (#1081) (#1266)
- [修正] CLI の --help が実装とずれていた 2 か所を正本から引く形に直した (#1248) (#1262)
- [修正] 信頼ダイアログの自動承諾がハイライトを見ずに Enter を送って claude を終了させる問題を直した (#1236) (#1260)
- [改善] 送達後に実行を断られた worker を完了と誤読しないようにした (#1034) (#1249)
- [修正] .agent の md にコンフリクトマーカーが commit されたら CI で落ちる番犬を足した (#1246) (#1255)
- [ドキュメント] agy のページで「--」が em ダッシュに変換されていたのを直した (#992) (#1254)
- [機能追加] 再起動後の復元で codex / agy も会話ごと戻るようにした (#1238) (#1245)
- [修正] 多重化が無いプラットフォームで無言の SSH 成功が畳まれないのを直した (#1137) (#1242)
- [ドキュメント] agent 別のセットアップガイドと縮退の明記を足した (#992) (#1251)
- [改善] #571 e2e が本番の事前信頼エントリを残さないようにした (#1022) (#1250)
- [修正] 番号なし・選択肢 2 つの信頼ダイアログを検知して respond できるようにした (#1223) (#1235)
- [修正] main の progress.md に混入したコンフリクトマーカーを除去した (#1247)
- [修正] remote_files のテストの約 1/350 フレークを直した (#1229) (#1244)
- [改善] テスト由来の事前信頼エントリの掃除口を用意した (#1030) (#1243)
- [改善] agy に実況 JSONL の一次シグナルを与え、完了検知を claude 同等にした (#1033) (#1241)
- [修正] cargo test が本番の data dir と HOME 配下の設定へ書かないようにした (#944) (#1232)
- [機能追加] tako setup のゼロスタート導入を codex / agy へ拡張した (#989) (#1239)
- [修正] Markdown プレビューが UTF-8 BOM を剥がさず先頭行の見出しを潰す問題を直した (#1202) (#1240)
- [機能追加] codex / agy worker から tako の MCP を呼べるようにした (#986) (#1237)
- [修正] 再起動後の claude resume が「起動途中の未検出」で自壊していたのを直した (#1076) (#1230)
- [修正] [Windows] マトリクスの tako_welcome / tako_ui_mode の過小申告を実挙動へ直した (#1204) (#1233)
- [修正] [Windows] UI のキー表記を正本経由にして Windows で実際のキーを出すようにした (#1203) (#1219)
- [修正] respond が生きているペインへ in-process 経路で届くようにした (#1200) (#1226)
- [ドキュメント] README に Windows のインストール手順を足した (#1206) (#1225)
- [修正] 一覧付与のコスト検査を実時間比較から量へ替え、番犬を tests 全体へ広げた (#1220) (#1227)
- [修正] リサイズでペインの cwd が起動時ディレクトリへ巻き戻るのを直した (#1199) (#1212)
- [修正] tako list の backend_windows を右パネルの表示状態から切り離した (#1191) (#1224)
- [機能追加] ターミナルのパスリンクに cmd+右クリックのコンテキストメニューを付けた (#1182) (#1215)
- [ドキュメント] 生成中ドットのプローブが指すセルフテスト項目番号を 134 へ直した (#1012) (#1221)
- [機能追加] 隔離・テストの tmux サーバーを所有者の生死で安全に回収できるようにした (#1192) (#1217)
- [修正] tmux resize --reset が実際に window サイズを戻すようにした (#1186) (#1218)
- [改善] 導入計画の権限説明を platform で出し分けた (#925) (#1216)

## [0.8.9] - 2026-09-09

Nightly patch release (automated). Changes since v0.8.8:
夜間パッチリリース（自動）。v0.8.8 以降の変更:

- [修正] tmux kill / resize の既定ソケットを list と揃え、生エラーを包んだ (#1190) (#1213)
- [改善] 区間を飛ばしたまま完成品を作らないようにした (#1081) (#1214)
- [改善] ホーム解決と `~` 短縮の入口をワークスペース全体で 1 本に寄せた (#893) (#1209)
- [修正] tmux open で取り込んだセッションが永久に掃除されない問題を根治した (#1188) (#1211)
- [改善] 解説動画の「かんたん表示」章を手順型デモへ差し替えた（11 区間 + 収録経路） (#1081) (#1184)
- [修正] codex の背景ターミナル待ちを idle + プロンプト未達と誤検知しないようにした (#1015) (#1198)
- [修正] 取り込みペインの tmux window を tako から選べるようにした (#1185) (#1201)
- [修正] Windows のメインスレッドのスタック予約量を宣言してセルフテスト項目 80 のクラッシュを根治した (#1133) (#1193)
- [修正] tmux cleanup が黙って何もしない形をやめ、--socket を実装した (#1187) (#1207)
- [修正] setup ディレクトリを data dir の境界へ寄せ、旧 Windows パスから自動移設した (#1019) (#1195)
- [修正] codex / agy の spawn に claude 語彙のモデル既定を渡さないようにした (#1013) (#1189)
- [修正] 解説動画のナレーションの読み間違いを全区間点検して直した（v4） (#1081) (#1183)
- [修正] セルフテストの tmux open 系の固定窓を状態待ち + 予算へ移送した (#1180) (#1181)
- [修正] 101c の引き継ぎ到達判定を後任の transcript から採るようにした (#1175) (#1179)
- [修正] 器つきペインの term-grid scroll を skip せず実測できるようにした (#1177) (#1178)
- [修正] 器つきペインの visual-test を完走させた（skip ではなく前提を作る側で直した） (#1173) (#1176)
- [修正] 実 claude e2e の待ちを固定窓から負荷追従の予算へ替えた (#771) (#1172)
- [修正] 器つきペインの visual-test カーソルラウンドを実測で切り分けて根治した (#1122) (#1174)

## [0.8.8] - 2026-09-08

Nightly patch release (automated). Changes since v0.8.7:
夜間パッチリリース（自動）。v0.8.7 以降の変更:

- [修正] セルフテスト項目 108 の高負荷フレークを窓ガードで根治した (#995) (#1170)
- [修正] 「追記ぶんだけ読む」の検査を実時間から読み出しバイト数へ替えた (#1167) (#1169)
- [ドキュメント] activeContext を起動時ロード予算（80 行）へ収め、2026-09-08 の実態へ更新した (#1139) (#1168)
- [修正] セルフテストの画面エコー待ちを固定窓から状態待ちへ寄せた (#1165) (#1166)
- [修正] セルフテストの高負荷フレーク 4 系統を状態待ちへ寄せた (#1153) (#1164)
- [修正] セルフテスト項目 102 の 25 桁ダイアログが検知できず 103 以降が走らない問題を根治した (#1162) (#1163)
- [修正] 仮想ディスプレイが列挙で空のとき、待たずにユーザーのメイン画面へ窓を開かないようにした (#1160) (#1161)
- [改善] master / solo の system prompt を起動時ロード予算の対象にして手順書へ分離した (#1154) (#1159)
- [修正] 夜間リリースを使い捨て worktree へ移し、共有ツリーを detached のまま放置しないようにした (#1136) (#1158)
- [ドキュメント] narrate.sh の音量段の説明を実装に合わせた（固定ゲインは #1156 で撤去済み） (#1081) (#1157)
- [改善] 解説動画のナレーションを VOICEVOX / ずんだもんへ差し替え、音量段を声に依存しない形へ直した (#1081) (#1156)
- [修正] 狭いペインの /model セレクタを選択肢ダイアログとして読めるようにした (#1143) (#1155)
- [改善] 解説動画の収録を常設の仮想ディスプレイ tako-vd へ移し、master 章を撮り直して v2 を合成 (#1081) (#1145)

## [0.8.7] - 2026-09-07

Nightly patch release (automated). Changes since v0.8.6:
夜間パッチリリース（自動）。v0.8.6 以降の変更:

- [修正] 仮想ディスプレイを Main にしたままにしない / 同名の増殖で止まる / 孤児の後片付けは実行条件を満たすまで拒否する (#1150) (#1152)
- [改善] プロファイルのリミット後自動復帰を master / solo 本人と後任 master にも配る (#1140) (#1151)
- [機能追加] 検証用 GUI をユーザーのメイン画面に出さない: 常設の仮想ディスプレイへ逃がす (#1141) (#1149)
- [改善] worker ペイン 1 枚に最小幅を保証し、割る spawn は別タブへ出す (#1132) (#1148)
- [修正] 狭いペインで折り返された選択肢ラベルを結合してから構造を組む (#1131) (#1144)
- [ドキュメント] AGENTS.md のコマンド表を .agent/commands.md へ分離する (#1139) (#1146)
- [機能追加] 起動時ロードの予算 tako context-budget: 計測・自動修正・規約の機械強制 (#1139) (#1142)
- [修正] tako が `claude auth login` を起こさない: 実機で 1 日 46 本まで積み上がった孤児を根治 (#1129) (#1135)
- [修正] 切断理由が折り返しで割れても認証系だと分かる: 鍵の失敗を打ち直さない / Windows 実機セルフテストが初完走 (#1127) (#1138)
- [ドキュメント] YouTube 解説動画の台本・収録・合成パイプライン (#1081) (#1126)
...
夜間パッチリリース（自動）。v0.8.4 以降の変更:

- [修正] remote scrollback を器の境界へ寄せ、ペイン ID も解決する (#972) (#1116)
- [修正] handoff の後任 master を前任と同じフォルダで起動する + profiles set --cwd (#1055 / #1056) (#1117)
- [修正] visual-test の 3 節が全節実行から呼ばれていない問題を根治 (#948) (#1110)
- [修正] Windows で古い claude の警告バナーが出ない問題を根治: 実行中プロセスのパス解決を境界へ (#936) (#1115)
- [修正] 器の同一性をソケット名の推測に頼らず明示する: セルフテスト項目 60 が特定の socket 名で落ちる問題を根治 (#1105) (#1113)
- [機能追加] codex / agy の「時間では解けない利用阻害」も同じ 1 本で検知する (#1107) (#1112)
- [修正] コマンド型の受け入れゲートを sh -c 決め打ちからシェル境界へ寄せる (#935) (#1111)
- [機能追加] 時間で解けない利用阻害 6 分類 / 8 文言を UsageLimit とは別の WorkerErrorKind として検知する (#1106) (#1109)
- [修正] Windows の SSH ペインが無言で死ぬ: 接続多重化を能力でゲートし、255 以外の失敗も理由を出す (#1090) (#1098)
- [修正] Windows で `canonicalize` の verbatim prefix が OSC 7 経路の cwd を壊す問題を根治 (#970) (#1101)
- [修正] プレビュー編集の自動保存が CLI / MCP の編集で発火しない問題を根治 (#973) (#1104)
- [修正] visual-test の全節実行が flicker で必ず落ちる問題を根治: 節が自分で場面を作る (#1083) (#1103)
- [修正] PWA e2e の承認カード 3 件が main で失敗する問題を根治: モックと assert を #425 の現行契約へ (#1089) (#1100)
- [修正] `You're out of usage credits` 系の別テンプレートを検知し、日付つきの解除時刻を丸めない (#1096) (#1099)
- [修正] セルフテスト項目 41 / 41b を $PROFILE の配置から切り離す: 専用ペインで OSC 7 / 133 を完結させる (#1091) (#1095)
- [機能追加] リモート刷新 柱3-F/G: スマホからのファイル編集と SSH 先のファイル操作 (#1084) (#1085) (#1097)
- [修正] 組織クレジット上限の見出しを検知できず自動復帰が発火しない問題を根治 (#1093) (#1094)
- [修正] Windows 実機の GUI セルフテスト完走を復旧: 壁 2 つ（OSC 7 の状態待ちと画面のリテラル期待）を潰す (#1092)
- [機能追加] リモート刷新 柱2-H: スマホから SSH の切り替え / 新規接続 (#1080) (#1087)
- [機能追加] リモート刷新 柱1: PWA から Claude 公式へ送り出す + スマホから master 起動 (#1077) (#1078) (#1088)
- [機能追加] リモートからファイルを見る・保存する API と PWA のファイルビュー (#1079) (#1086)
- [修正] visual-test の remote-tree 節を #1041 の RemoteFolder へ追従 + feature 付き lint を CI へ (#1072) (#1082)
...
夜間パッチリリース（自動）。v0.8.1 以降の変更:

- [ドキュメント] 8/29 並行 8 本の progress 追記 + activeContext を規約サイズへ圧縮 (#1054)
- [改善] 自己検査を持たない daemon の serve_ok: null に理由を添える (#1049) (#1053)
- [修正] remote が「起動中」を名乗ったまま tailnet から見えない問題を根治 (#1049) (#1051)
- [修正] セルフテスト項目 111 の番兵と実 claude の使用率の衝突で確定失敗する問題を根治 (#1048) (#1052)
- [修正] ネット断で SSH のペイン・タブ・リモートフォルダを失わず回線復帰で自動再接続する (#1040) (#1050)
- [ドキュメント] #1038 の作業履歴を progress へ追記 (#1038) (#1047)
- [修正] アプリ内アップデートで Dock のピン留めが外れる問題を根治 (#1042) (#1046)
- [修正] remote の中継先をループバック TCP へ寄せて全リクエスト 502 を根治する (#1038) (#971) (#1044)
- [機能追加] Finder からターミナルペインへの D&D でパスを挿入する (#1043) (#1045)
- [修正] 検索のヒット位置が小文字化でバイト長が変わる文字でずれる問題を根治 (#1016) (#1039)
- [機能追加] ファイルツリーに git ステータスの色分けとバッジを出す (#1009) (#1037)
- [ドキュメント] 3 CLI の北極星測定レポートを追加 (#983) (#975) (#1036)
- [ドキュメント] docs 全体に実画面のスクリーンショットを入れる（22 図 / 12 ページ）(#1027) (#1032)
