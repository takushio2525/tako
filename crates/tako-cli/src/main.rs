...
fn sleep_guard_local(sub: &SleepGuardCommand) -> Result<(), String> {
    match sub {
        SleepGuardCommand::Status => {
            ...
            if state.thermal_state != tako_control::sleep_guard::ThermalState::Nominal
                || tako_control::sleep_guard::lid_requires_privileged_setup()
            {
                // thermal は macOS でしか取れない。Windows で常に nominal と出しても情報がない
        ...
        SleepGuardCommand::InstallLidSleep => {
            // 初回セットアップの要否は OS で違う（macOS = sudoers 登録、Windows = 不要）。
