...
//! ここではその単位になる子ビューを 3 種類だけ用意する。
//!
//! - [`PaneBody`]: ペイン 1 枚の本体
//! - [`PaneHeader`]: ペイン 1 枚のタイトルバー（#803）
//! - [`Chrome`]: タブバー / サイドバー / 右パネル / ステータスバー
//!
//! どれも状態は持たず、描画の実体は `TakoApp` 側の既存メソッドをそのまま呼ぶ
...
//!
//! ## `cached` は入れ子にできない（#801 の実測 → #803 で回避）
//!
...
//!
//! #803 はこれを「ヘッダをルート側の兄弟へ持ち上げる」ことで回避した。ヘッダは
//! [`PaneHeader`] として**ペイン本体と同列**（どちらもルートの子）に置かれ、本体は
//! ヘッダの場所を同じ高さの空きスペーサーで空ける。ヘッダに出るもの（タイトル・
//! role・状態ドット・cwd・workers）は PTY 出力では変わらないので、出力フレームでは
//! prepaint / paint ごと再利用される（実測は `.agent/architecture.md`）。
//!
...
/// ビューキャッシュが有効か（セルフテスト 108 / 110 の前提確認用）
pub(crate) fn enabled() -> bool {
    !view_cache_disabled()
}

/// ヘッダのキャッシュだけを切る逃げ道（`TAKO_803_NO_HEADER_CACHE=1`）。
///
/// #803 の効果を同じバイナリで A/B するためのもの。**持ち上げた構造はそのまま**に
/// 「ヘッダを毎フレーム作り直す」= 持ち上げ前と同じ仕事量へ戻す
/// （ペイン本体・クロームのキャッシュは効いたままなので、差はヘッダぶんだけになる）
pub(crate) fn header_cache_disabled() -> bool {
    static OFF: std::sync::OnceLock<bool> = std::sync::OnceLock::new();
    *OFF.get_or_init(|| std::env::var_os("TAKO_803_NO_HEADER_CACHE").is_some())
}
...
pub(crate) fn cached_view<T: Render>(
    ...
    cached_view_when(!view_cache_disabled(), view, style, cx)
}

/// キャッシュするかどうかを呼び出し側が決める版（#803 の A/B 用）。
/// `cache = false` なら同じスタイルの箱に入れて毎フレーム描き直す
pub(crate) fn cached_view_when<T: Render>(
    cache: bool,
    view: &Entity<T>,
    style: StyleRefinement,
    cx: &gpui::App,
...
/// ペイン 1 枚のタイトルバーを描く子ビュー（#803）。
///
/// [`PaneBody`] の**兄弟**（どちらもルートの子）として、ペイン矩形の上端へ絶対配置で
/// 置かれる。本体側は同じ高さのスペーサーでこの場所を空けるので、積み上げ
/// （`pane_text_areas` の会計）は持ち上げの前後で変わらない。
///
/// 自前の状態は持たず、描画は `TakoApp::render_pane_header` に委譲する。
...
impl Render for PaneHeader {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let Some(app) = self.app.upgrade() else {
            return div().into_any_element();
        };
        let pane = self.pane;
        app.update(cx, |app, cx| app.render_pane_header(pane, cx))
    }
}
