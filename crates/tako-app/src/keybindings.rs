...
///
/// Windows には GPUI のメニューバーが無く、ドキュメントを読まないと
...
/// macOS 以外（Windows / Linux）向けの追加バインド（#517 / #585）
///
...
///   拡大は正規化後の字面 `ctrl-+` で書く
///
/// ## ペイン分割に **Ctrl+D 単独を使わない**理由（ユーザーからの名指し要望）
///
...
fn platform_bindings() -> Vec<KeyBinding> {
    vec![
        ...
        // アプリの終了。**Windows には正規の終了手段が無い**: `handle_window_close` は
        // 最後の 1 枚でもプロセスを終了させず（macOS の Dock 復帰前提）、`gpui_windows` も
        ...
        // まま復帰も終了もできないプロセスを残す。Ctrl+Q は XON（フロー制御）なので
        // shift 段へ逃がす
...
/// 貼り付けのキーストロークか（#546）。
/// `key_bindings()` の `PasteClipboard` バインドと同じ組み合わせだけを真にする。
/// アプリ内テキスト入力がキーを消費している間はアクションが発火しないので、
/// `handle_key` はこの判定で `paste()` へ回す。
///
/// **非 macOS では `cmd-v` が届かない**（GPUI の `cmd` は platform 修飾 = Win キーで、
/// Win+V は OS のクリップボード履歴に奪われる。#585）ため、
/// [`platform_bindings`] が張る `ctrl-v` / `ctrl-shift-v` / `shift-insert` を見る。
/// 判定とバインド表がずれると入力欄でだけ貼り付けが死ぬので、
/// `貼り付け判定はペーストのバインドと一致する` テストが両者を突き合わせている
pub(crate) fn is_paste_keystroke(ks: &Keystroke) -> bool {
    let m = &ks.modifiers;
    if cfg!(target_os = "macos") {
        return ks.key == "v" && m.platform && !m.control && !m.alt;
    }
    // platform（Win）修飾つきは OS に奪われて届かないので真にしない
    if m.platform || m.alt {
        return false;
    }
    (ks.key == "v" && m.control) || (ks.key == "insert" && m.shift && !m.control)
}
...
mod tests {
    ...
    /// #585: 非 macOS では `cmd-v`（= Win+V）が OS に奪われて届かないので、
    /// [`platform_bindings`] が張るキーを [`is_paste_keystroke`] も見ていること。
    /// ここがずれると「入力欄にフォーカスがあるときだけ貼り付けが死ぬ」（#546 と同型）
    #[test]
    #[cfg(not(target_os = "macos"))]
    fn 貼り付け判定は非macosのペーストキーを拾う() {
        for spec in ["ctrl-v", "ctrl-shift-v", "shift-insert"] {
            assert!(
                is_paste_keystroke(&Keystroke::parse(spec).expect(spec)),
                "{spec} を貼り付けと判定していない"
            );
        }
        // 修飾なしの v は本文入力
        assert!(!is_paste_keystroke(&ks_char("v", "v")));
        // OS に奪われる platform 修飾つきは真にしない（届かないので拾っても無意味）
        assert!(!is_paste_keystroke(
            &Keystroke::parse("cmd-v").expect("cmd-v")
        ));
        // 別バインド / 別キー
        for spec in [
            "ctrl-c",
    ...
    /// #546 / #585: [`is_paste_keystroke`] とバインド表が食い違っていないこと。
    /// 判定を手書きで持っている以上、`PasteClipboard` の単発バインドは
    /// **全部**この判定に拾われなければならない（拾えないキーは入力欄で死ぬ）
    #[test]
    fn 貼り付け判定はペーストのバインドと一致する() {
        let mut checked = 0usize;
        for b in key_bindings() {
            if b.action().name() != "tako::PasteClipboard" {
                continue;
            }
            let ks: Vec<Keystroke> = b.keystrokes().iter().map(|k| k.inner().clone()).collect();
            // 単発キーストロークのバインドだけが handle_key の判定を通る
            if ks.len() != 1 {
                continue;
            }
            let k = &ks[0];
            // 非 macOS の platform 修飾つきは OS に奪われて届かない（判定対象外）
            if !cfg!(target_os = "macos") && k.modifiers.platform {
                continue;
            }
            assert!(
                is_paste_keystroke(k),
                "{:?} は PasteClipboard のバインドなのに is_paste_keystroke が拾わない",
                k
            );
            checked += 1;
        }
    ...
    fn cmd_qはコンテキスト述語なしでquitにバインドされている() {
        ...
        // 非 macOS では `ctrl-shift-q` も張る（#585）。ここで見たいのは cmd-q の側なので
        // platform 修飾を持つバインドに絞る
