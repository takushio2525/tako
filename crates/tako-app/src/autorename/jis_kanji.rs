//! 日本語で使われる漢字の集合（自動リネームの出力バリデーション用。#552）
//!
//! 用途は 1 つだけ: 自動命名の結果に**日本語では使わない漢字**（簡体字の
//! `开` `发` `图` など）が混ざっていないかを判定する。日本語 UI なのに
//! 中国語の字体が出る事故（#552 で実観測: `开発`）を防ぐ。
//!
//! 集合は Windows-31J（CP932）で符号化できる CJK 統合漢字 = JIS X 0208 の
//! 第 1・第 2 水準 + NEC / IBM 拡張。日本語の実用漢字をほぼ網羅し、
//! 簡体字専用の字体は（`无` のような日中共通の異体字を除き）含まない。
//! 依存クレートを増やさないため、集合そのものをここへ埋め込む。
//!
//! 再生成（Python 標準ライブラリの cp932 コーデックから機械生成する）:
//!
//! ```text
//! python3 -c '
//! out = []
...
/// CP932 で表現できる CJK 統合漢字（コードポイント昇順）。
/// 判定は `CP932_KANJI.contains(ch)`（20KB の線形走査だが、呼ばれるのは
/// リネーム 1 回あたり最大 24 文字ぶんなので実測誤差レベル）
pub const CP932_KANJI: &str = "一丁七万丈三上下不与丐丑且丕世丗丘丙丞両並丨个中丱串丶丸丹主丼丿乂乃久之乍乎乏乕乖乗乘乙九乞也乢\
