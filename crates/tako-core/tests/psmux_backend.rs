...
// #686（器の copy mode ゲート）の 2 本 — `copy_mode滞在中の打鍵がin_band解除で届く` と
// `器のホイールは上下対称で最下部でcopy_modeを抜ける` — は **スライス 2b へ送った**。
//
// どちらも `TerminalSession::wheel_scrolled_back` / `arm_copy_mode_exit` を使うが、
// main の `terminal.rs` は #817 で自前の `pty_loop` へ置き換わっており、これらの
// メソッドがまだ無い（win467 の実装は upstream の `EventLoop` 前提なので移植ではなく
// 再実装が要る）。器側の契約（`pane_in_mode` / `copy_mode_exit_bytes`）は
// 本スライスに入っているので、2b で消費側を実装するときにこの 2 本を戻す。
//
// **macOS では気づけない**（2 本とも `#[cfg(windows)]`）。Windows 実機の
// `cargo test` で E0599 になって初めて分かったので、2b では実機ビルドを先に通すこと。
