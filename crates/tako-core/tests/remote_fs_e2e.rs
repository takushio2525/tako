...
///
/// **Windows だけ skip**（#930）。Windows では分類そのものは正しいのに
/// **接続経路が戻ってこない**枝がある（実機実測: 536 秒ぶら下がり、`ssh` を
/// 落として初めて `ok` になった）。#1278 で CI の Windows を blocking にしたので、
/// ここでぶら下がると run 1 本を丸ごと失う。根治は #930 / #1090（ControlMaster の
/// Windows 対応の設計判断）で、直したらこの `ignore` を外す
#[test]
#[cfg_attr(
    windows,
    ignore = "Windows は接続経路が戻ってこない枝がある（分類は正しい。#930 / #1090）"
)]
