...
pub struct HeaderVisibility {
    ...
    /// 実行ペインの実行中 / 完了 / 失敗のバッジ（#1657）。**結果が伝わらないと困る**ので
    /// `ssh_connect` と同じ早い段階から出す（右へ細く割った実行ペインでも消えない）
    pub run_badge: bool,
    pub shell_info: bool,
...
impl HeaderVisibility {
    ...
    /// ssh_connect（#1010）と run_badge（#1657）は結果が伝わらないと困るので早くから出す
    /// （badge の次に残る）
