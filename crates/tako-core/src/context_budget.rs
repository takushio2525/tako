...
///
/// 実測（2026-09-23 / 152 本）は最小化 JSON で 198,830 バイト・121,852 文字。
/// 実トークナイザ（tiktoken `o200k_base`）で **51,076 トークン**
/// （3.89 バイト / トークン）。JSON の構造と ASCII のキーが多いぶん、
/// 日本語主体の文書へ合わせた [`estimate_tokens`] は約 1.8 倍に出る。
/// **だから上限はトークンではなくバイトで置く** — バイトはトークナイザに依らない。
///
/// #1539 の初期値 210 KB は圧縮前（201,535 バイト）の + 6.7% だったので、
/// #1540 の圧縮後の実測で **200 KB へ締め直した**（余地 5,970 バイト = + 3.0%）。
/// 余地は 1 本あたりの平均 1,308 バイトでツール 4〜5 本ぶん。**そこを超えたら
/// ツールを隠すのではなく 1 本ずつの説明文を短くする**（圧縮の規則は
/// `.agent/conventions.md`「MCP カタログの説明文は AI が使える情報だけ載せる」）。
pub const MCP_CATALOG_MAX_BYTES: usize = 200 * 1024;
