...
pub enum Request {
    ...
    /// 直接ペインのスクロールバック保持上限（Issue #818）。
    /// `lines` 省略時は現在状態の取得のみ。設定は永続化され、
    /// **生存中のペインにもその場で効く**（下げれば履歴が切り詰められて RAM が戻る）。
    Scrollback { lines: Option<usize> },
    ...
    TreeFolder {
        /// "add" / "remove" / "list" / "roots" / "git-status"
        ///
        /// "roots" は**全タブ**のツリールート（各ペインの cwd + ピン留めフォルダ）を
        /// 返す（#1079。リモートのファイル API の認可の正）
        action: String,
    ...
    ///   #1041: ツリーの**先頭**（ローカルより前）に出し、同じタブへ SSH 済み +
    ///   `cd` 済みのターミナルペインも用意する（`terminal: false` で抑止）
    ...
    RemoteFolder {
        ...
        #[serde(default)]
        tab: Option<u64>,
        /// 新しく作ったペイン・タブへフォーカスを移すか（省略時 true）。
        /// `open-file` / `ssh-pane` と、#1041 で `open` が用意するターミナルに効く
        #[serde(default)]
        focus: Option<bool>,
        ...
        #[serde(default)]
        enabled: Option<bool>,
        /// `open` でターミナルも同じホストへ繋ぐか（#1041。**省略時 = true**）。
        ///
        /// フォルダを開いたら SSH 済み + そのフォルダへ `cd` 済みのペインが同じタブに
        /// 用意される（VSCode Remote 相当）。同じホストへ繋がった生きたペインが
        /// あれば作らない。`false` にすると開くだけ（あとから
        /// `remote-folder ssh-pane` で繋げる）
        #[serde(default)]
...
pub enum FileOpKind {
    ...
    OpenWith,
    /// **tako の中で開く**（#1182）。ファイル = プレビューペイン / ディレクトリ =
    /// そのディレクトリでシェルを起動するペイン、どちらも `pane` を右へ分割して生やす。
    ///
    /// ターミナル内のパスリンクの cmd+クリック（#147 / #153）とパスメニューの
    /// 「tako で開く」が**この 1 実装を通る**。実体は `OpenFile` / `Split` への
