...
mod tests {
    ...
    // POSIX の mode ビットで「読めるが書けない」ディレクトリを作るので unix 限定。
    // Windows で同じ状態を作るには ACL が要る（別物なので同じテストにはできない）。
    // **cfg を付け忘れると `tako-control` のテストバイナリが Windows でコンパイルできず、
    // スイート全体が 1 件も走らなくなる**（作法 11。#1063 の実機検証で踏んだ）
    #[cfg(unix)]
    #[test]
