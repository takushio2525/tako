...
fn check_health(host: &dyn ControlHost) -> Value {
    ...
    // 文と重さの正は `Placement::health_issue`（#1160。GUI を立てずに検証できる形へ
    // 寄せてある = ここで組み立てると自己テストが通らないと固定できない）
    if let Some(issue) = display_placement.as_ref().and_then(|p| p.health_issue()) {
        issues.push(issue);
    }
    ...
    json!({
        ...
        // #1160: `refused=true` なら**窓を 1 枚も開いていない**（検証用 GUI が置き先を
        // 用意できず、ユーザーの画面へ落ちる代わりに終了した）。`retries` は列挙を
        // やり直した回数で、`> 0` なら「起動の瞬間だけ列挙が空だった」を拾っている
