...
///
/// 値の型を固定しないのは、対応付けの宛先が経路で違うため（器ありは
/// `session:window.pane` の文字列、器なしは tako のペイン ID = u64。#728）。
/// 辿り方は同じなので、ここを 2 本に分けると片方だけ直す事故が起きる
...
pub fn any_target_has_running_children(sessions: &[&str], pane_pids: &[u32]) -> bool {
    ...
    let parents = process_parent_map();
    if !panes.is_empty() && count_sessions_with_children_inner(sessions, &panes, &parents) > 0 {
        return true;
    }
    pane_pids
        .iter()
        .any(|pid| has_children_of_pid_inner(*pid, &parents))
}
