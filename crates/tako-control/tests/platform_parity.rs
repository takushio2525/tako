    ...
    const BASELINE: &[(&str, usize, &str)] = &[
        ...
        (
            ...
            "macOS 限定の `ditto`（zip 展開）1 箇所。Windows インストーラーの起動は \
             ...
             もう 1 件は番犬（#1253）が**自分と同じテストバイナリ**を偽 brew つきの \
             PATH で起こす `#[cfg(test)]` の 1 箇所（製品の起動ではない）",
        ),
        (
        ...
        (
            ...
            7,
            "`/bin/sh` / `/bin/sleep`（`#[cfg(unix)]` とテスト。#1768 の daemon の子の fd 検査で \
             1 件増えた）。ゾンビ判定の `/bin/ps` は              #1067 で境界（`platform::process::is_zombie`）へ移し、抑止を通している",
        ),
        (
            "crates/tako-control/src/sleep_guard.rs",
            6,
        ...
        (
            "crates/tako-core/src/platform/shell.rs",
            3,
            "テストモジュール内（#1031。組み立てた sh 片を実際に走らせて「成功時は待たない」\
