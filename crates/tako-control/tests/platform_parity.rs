...
fn コンソール窓を抑止していない子プロセス起動が増えていない() {
    ...
    const BASELINE: &[(&str, usize, &str)] = &[
        ...
        (
            ...
            1,
            "ログインシェル経由の探索（#[cfg(unix)]）",
        ),
        (
            "crates/tako-app/src/update_checker.rs",
            5,
            "テストモジュールと、Windows インストーラーの起動（GUI アプリなので窓を見せるのが正）",
        ),
        (
            "crates/tako-cli/src/setup.rs",
            3,
            "macOS 限定（brew / claude の対話起動）。対話子は端末を継承させる必要がある",
        ),
        (
        ...
        (
            ...
            3,
            "`hostname`（#[cfg(unix)]）と macOS 限定の OS 情報取得",
        ),
        ("crates/tako-core/src/git.rs", 5, "テストモジュール内"),
        (
