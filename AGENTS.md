...
| UI 表示モード切替（GUI ライク表示。#691/#694/#702） | `tako ui-mode [gui\|terminal\|toggle]`（引数なしで現在値。既定 terminal = 従来の表示。gui ではアイドルなシェルのペインが「AI チームに任せる / AI と 1 対 1 で話す / コマンド入力へ」の 3 ボタンになり、**claude 対話ペインは会話ビュー**（モデル名・状態・コンテキスト残量バー・md 描画・ツール / 思考の折りたたみ・下端追従。worker は read-only）になる。settings.json 永続化・全ウィンドウ即時反映。タブバーのテーマボタン左隣 / ⌘K パレット / MCP `tako_ui_mode` と 1:1）<br>`tako ui-mode release [--pane N]` = そのペインだけターミナル表示へ（揮発。`restore` で戻す。会話ビューのヘッダ「ターミナルを表示」と同じ）。**表示レイヤだけの切替なので PTY・tmux セッション・実行中プロセスには影響しない** |
