...
| **検証用 GUI の置き場（ユーザーの画面に窓を出さない。#1141）** | `scripts/lib/virtual-display.sh ensure`（常設の仮想ディスプレイ `tako-vd` を用意。冪等・**消す機能は無い**。`bounds` / `status` / `move-window <pid>` も持つ）。**以後 `TAKO_ISOLATED` / `TAKO_SELF_TEST` / `TAKO_VISUAL_TEST` の起動は指定なしでそこへ出る**（面を選ぶなら `TAKO_DISPLAY=<名前\|UUID\|index>`）。GPUI は**窓が完全に隠れると描画を止める**（#470）ので別 Space への退避は代わりにならず、OS から実ディスプレイとして見える面が要る。指定が外れても起動は止まらず persist.log に理由 + 候補が 1 行残り、どこへ置いたかは MCP `tako_check_health` の `display_placement` で読める。**`tako-vd` は消さない**（常設）。器（BetterDisplay）の応答文は当てにならないので判定は NSScreen が正 = `.agent/conventions.md`「窓を出す検証はユーザーの画面に出さない」 |
