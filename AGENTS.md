...
| **起動保証の確認（#665）** | `tako orchestrator launch-status --pane <N> [--worker <ID>] [--wait]`（シェル起動 → 起動コマンド → CLI 起動 → プロンプト送達 のどこまで確認できたか。MCP `tako_orchestrator_launch_status` と 1:1） |
...
| メニューバー操作（#657） | `tako menu list` / `open <メニュー名>` / `close` / `invoke "ファイル/新規タブ"`（Windows はタブバーの一段上に自前描画のメニューバー行 + 右端にウィンドウコントロール。GUI では F10 → ← → ↑ ↓ / Enter / Esc。macOS はメニューが OS のメニューバーに載るので open / close は不可、list / invoke のみ。MCP `tako_menu` と 1:1） |
...
| **doc サイトの Windows 対応状況ページを再生成（#591）** | `node scripts/gen-windows-support-docs.mjs`（`--check` で差分検査のみ）。**`support.rs` の対応状況を変えたら必ず実行する**（ページは生成物。手で編集しない） |
