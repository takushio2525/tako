...
worker への分担が要らず、AI と 1 対 1 で直接作業したいだけのときは `tako solo` を使います。オーケストレーション（worker の起動）を**あえて禁止**し、その AI 自身が手を動かして——ファイルを読み、コードを書き、テストを走らせ、コミットまで——直接こなします。
...
```bash
# 1. セットアップ（未実施なら。検出・前回値・既定値で質問ゼロ）
tako setup

# 2. master を起動（1 対 1 でよければ tako solo）
tako master
```
