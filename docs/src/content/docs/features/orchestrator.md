...
独立した作業なら、master は worker を複数同時に立ち上げて並列に進めます。
...
master の実体は Claude Code（`claude` コマンド）です。未インストール・未ログインの場合は先に `tako setup` を実行するか、[Claude Code のドキュメント](https://docs.anthropic.com/en/docs/claude-code)に従って導入してください。
...
- **worker の起動**: master は依頼から対象プロジェクトを判断し、そのディレクトリで claude を新しいペインに起動して指示を渡します
...
| `profiles/<名前>.yaml` | master / worker のモデル・effort・worker ポリシー。**起動設定の唯一の正** |
