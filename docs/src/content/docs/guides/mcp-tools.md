...
| `tako_lsp` | 言語サーバの言語機能（`action` = diagnostics で編集中のコードの診断。`pane` / `severity` で絞る） |
...
| ツール名 | 説明 |
|---|---|
| `tako_run` | ファイルを実行する（`tako:run` 宣言・プロジェクト既定・拡張子既定で新ペイン分割。同じファイル・プロファイルの実行ペインはその位置で差し替える。`new_pane` で増やせる） |
