...
| ツール名 | 説明 |
|---|---|
| `tako_run` | ファイルを実行する（`tako:run` 宣言・プロジェクト既定・拡張子既定で新ペイン分割。同じファイル・プロファイルの実行ペインはその位置で差し替える。`new_pane` で増やせる。プレビューの未保存の編集は走らせる前に保存する） |
...
| `tako_run_interactive_status` | `run_interactive` / `run` で起動したペインの完了状態を確認する（`auto_close` で閉じたあとも終了コードを返す） |
