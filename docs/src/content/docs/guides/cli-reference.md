...
| [`jump`](#tako-jump) | 行を指定して開いた位置を戻る・進む（ジャンプ履歴） |
...
`-- <コマンド>` で実行するコマンドは、**普通にペインを開いたときと同じ環境**（ログインシェル + `~/.zshrc` などの設定ファイルを読んだ後の PATH）で動きます。nodebrew / fnm / Homebrew で入れた `npm` や `node` も、ターミナルで打つときとまったく同じように見つかります。

コマンドが**失敗したときだけ**、ペインは閉じずに終了コード（`__TAKO_EXIT=<コード>`）と案内を残して止まります。Enter を押すとペインが閉じます。成功したコマンドは今までどおり終わった時点でペインごと閉じるので、そのときの出力は `tako logs show <ペイン ID>` で後から読めます。
...
画面の中で **Cmd+クリック（Windows は Ctrl+クリック）で開けるもの**（URL・実在するパス）を列挙します。GUI のクリック判定と同じ経路を通るので、「この画面のこのパスはリンクになるのか」を人手のクリック無しに確かめられます。`--text` を渡すと画面の代わりにその文字列を材料にするため、画面の写しを貼って判定を再現できます。
...
| `--line <行>` / `--column <桁>` | 開いた直後にその行へ飛ぶ（1 始まり。表示はソースになる）。[ジャンプ履歴](#tako-jump)に積まれる |

### tako jump

行を指定して開くと（`tako open <file> --line <行>`、MCP の `tako_open_file` の `line`）、**開く前にいた場所**と**着地点**がジャンプ履歴に積まれます。定義へ飛んだあとで元の場所へ戻るための機能です。GUI では **⌃-** で戻り **⌃⇧-** で進みます（Windows は **Ctrl+Alt+←** / **Ctrl+Alt+→**。Ctrl+- は縮小に使っているため）。

```bash
tako jump back       # 1 つ戻る
tako jump forward    # 1 つ進む
tako jump list       # 履歴の全項目と現在位置（> が現在位置）
```

- 行を渡さない `tako open`（ファイルツリーで眺めるなど）は積みません
- 同じファイルの同じ行へ続けて飛んでも 1 つにまとまります。履歴は最大 100 件で、古いものから消えます
- **閉じたペインの項目**へ戻ると、そのファイルを開き直します（同じタブのプレビューを使い回し、無ければ右に分割）
- **消えたファイル**の項目は読み飛ばして履歴から消します
- 端まで来たら何もしません（「これより前の履歴は無い」）
- `--focus` で着地したペインへフォーカスを移します（GUI のキーは常に移します）

MCP では `tako_jump`（`action` = `back` / `forward` / `list`）が同じ操作です。履歴は tako を終了すると消えます。
