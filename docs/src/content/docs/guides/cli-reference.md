...
接続リンク / QR は常に **`https://tako-remote.pages.dev` の固定 URL** です（トンネル有効時）。
ブックマークしておけば、トンネルの再起動や Mac の再起動後も同じ URL から再接続できます
（裏側のトンネル URL はリレーが自動解決します）。リレー障害時のための予備リンク
（トンネル直 URL）も同時に表示されます。

**既定では暗号化トンネル（cloudflared）経由でのみホスト**します。cloudflared が無い等で
トンネルを張れない場合は、暗号化されていない状態で公開しないために**起動を拒否**します
（`brew install cloudflared` で導入してください）。信頼できる LAN 内で平文のまま使いたい
場合に限り、`--insecure` で平文 HTTP の LAN 直モードを明示的に有効化できます（非推奨）。

```bash
# サーバーを起動し、接続用 QR コードを表示（暗号化トンネル必須）
tako remote start

# 平文 LAN 直モード（明示 opt-in・非推奨。信頼できる LAN 内でのみ）
tako remote start --insecure
...
- **エンドツーエンド暗号化ではありません。** トンネル経由（cloudflared・既定）は HTTPS ですが Cloudflare のエッジで一度復号されます。
- **既定では暗号化トンネル経由でのみホスト**し、トンネルを張れない場合は起動を拒否します。**`--insecure`（LAN 直モード）は平文 HTTP** で、同じ Wi-Fi / LAN 上の第三者に通信（トークンを含む）を盗聴されうるため、信頼できるネットワークでのみ明示的に使ってください。起動時にも警告が表示されます。
:::

:::note[リレーの仕組みと差し替え]
トンネル URL の再解決（2 回目以降の QR 再スキャン不要化）には、作者がベストエフォートで運用する公共リレー（Cloudflare Workers・SLA なし）を使います。リレーに保存されるのは machineId とトンネル URL のみで、**画面内容やトークンは通りません**（それらはトンネル経由で tako 本体と直接やり取りされ、トークン認証で保護されます）。リレーへの登録は端末ごとに自動生成されるシークレットで first-write-wins 保護され、送信元 IP ごとのレート制限も入っています。`TAKO_RELAY_URL` 環境変数で自前のリレーに差し替えることもできます（構築手順はリポジトリの `web/tako-remote-worker/README.md`）。
:::
