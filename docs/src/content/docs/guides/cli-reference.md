---
title: CLI リファレンス
description: tako コマンド全 68 種の逆引き一覧 — 目的・使い方・実行例・よく使うオプション
---

`tako` CLI は、ターミナルの画面操作（ペイン分割・テキスト送信・レイアウト変更など）をコマンドとして実行するためのツールです。シェルスクリプトからの自動化にも、AI エージェントからの操作にも使われます。

トップレベルのコマンドは **68 種**、サブコマンドまで含めると 223 種あります。ほぼすべてが同名の MCP ツールと 1:1 で対応しており（[MCP ツール一覧](/guides/mcp-tools/)）、人ができる操作は AI も同じ経路で実行できます。
...
やりたいことから引くための全 68 コマンドの一覧です。詳細のあるものはリンクから飛べます。
