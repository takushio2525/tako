...
初回は `tako remote setup` で対話的にセットアップします。

```bash
# 対話ウィザード（Tailscale 導入 → ログイン → HTTPS → serve 設定 → QR 生成）
tako remote setup
...
Tailscale が未セットアップの場合、`tako remote start` は不足項目を列挙して
`tako remote setup` への実行を案内します。

### GUI からの起動（コマンドを覚えなくてよい）

ステータスバーの「リモート」チップは**常に表示されています**（停止中は中空の丸 +
「リモート オフ」、稼働中は塗りつぶしの丸 + 接続端末数）。停止中にクリックすると
起動パネルが開き、**「リモートを起動」ボタン**で `tako remote start` と同じ起動が
できます。Tailscale の設定が足りないときは不足項目とセットアップコマンドがそのパネルに
出るので、理由が分からないまま止まることはありません。稼働中のクリックは従来どおり
接続端末の一覧・接続 URL・全遮断（kill switch）です。

### 二層認証（機器ペアリング）

接続には **2 つの認証層**があります。

1. **層① Tailscale identity** — tailnet にログインした端末だけがサーバーに到達できます
   （Tailscale が保証する identity をサーバー側で検証。tailnet 外・偽装は届きません）。
2. **層② 機器ペアリング** — tailnet 内の端末でも、**Mac 画面で承認するまで画面データを
   一切受け取れません**。スマホで固定 URL を初めて開くと、Mac の画面に承認ダイアログが出て、
   端末名と要求権限を確認してから許可します。
...
**承認と権限の変更は Mac の画面でしか行えません**（AI・CLI・スマホからは要求だけ）。
これは「tako を操作できる端末を増やす」というセキュリティ境界の操作を、
必ず Mac の前にいる人間の手を経由させるための設計です。

接続 URL / QR は**恒久固定で、secret（トークン）を含みません**。QR を他人に見られても、
その端末は Mac での承認を通らない限り何も見られません（従来の URL 埋め込みトークンは廃止）。

```bash
# サーバーを起動し、接続用 QR コードを表示（Tailscale セットアップ済みが前提）
# 初回アクセス時に Mac 画面へ承認ダイアログが出ます
tako remote start

# 状態確認（恒久固定 URL・登録端末数を表示。secret は含まれません）
tako remote status
tako remote stop

# ペアリング済み端末の一覧・失効（失効すると接続中の端末は即座に切断されます）
...
稼働中は Mac の status bar に接続端末数のインジケータが出ます。クリックすると
接続中の端末一覧・失効・**kill switch（全端末を即遮断）**を操作できます。

:::caution[セキュリティ上の注意]
リモートアクセスは**正規の遠隔操作ツール**です。Interact 以上の権限を許可した端末は、
リモートのブラウザから**あなたのターミナルへ任意のコマンドを送信できます**（実質的にシェルへの
フルアクセス）。自分の端末を自分で操作する目的でのみ使ってください。

- **知らない端末の承認ダイアログが出たら「拒否」してください。** 承認するまで画面は一切
  見られません。心当たりのある自分の端末だけを、必要最小限の権限で許可してください。
- **到達できるのは同じ tailnet にログインした端末だけ**です。それでも tailnet 内の全端末が
  信頼できるとは限らない場合（共有 tailnet 等）は、この機能を使わないでください。
- **Tailscale アカウントが乗っ取られても、機器ペアリング層が最後の砦になります**（承認していない
  端末は入れません）。あわせてアカウントの 2 要素認証と tailnet lock を有効にしてください。
:::
