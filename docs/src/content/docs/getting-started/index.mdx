...
Windows 対応は現在進行中で、まずは**ターミナルとしての基本機能**（タブ・ペイン分割・日本語入力・`tako` CLI）から段階的に対応しています。tmux バックエンドによる完全復元、スマホからのリモート接続、ファイルプレビュー / Web ビューなどはまだ対応していない、または挙動が限定されます。

**いま動かしている環境で何が使えるか**は、インストール後に次のコマンドで確認できます。
