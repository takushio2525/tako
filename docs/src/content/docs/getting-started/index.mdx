...
tako を使い始めるまでの手順を、前提知識がない方でも上から順に読めば動かせるように説明します。所要時間は 10 分程度です。macOS と Windows の両方に対応しており、手順が分かれるところは**タブで OS を切り替えて**読めます。

## 全体の流れ

1. **tako 本体をインストールする**（macOS は Homebrew / ZIP、Windows はインストーラー / ZIP）
2. **tako を起動する**
...
| もの | 必須？ | 説明 |
|---|---|---|
| macOS または Windows | 必須 | **macOS**: Apple Silicon（M1 以降）。**Windows**: 64bit（x64）の Windows 10 バージョン 1809 以降、または Windows 11。Windows 版は**テスター向けのプレビュー**です（後述） |
| [Homebrew](https://brew.sh/ja/) | macOS で推奨 | macOS 用のアプリ管理ツール。インストールとアップデートが 1 コマンドで済みます |
| AI エージェント CLI | AI 連携に1つ以上必要 | `claude`（Claude Code）/ `codex`（OpenAI Codex CLI）/ `agy`（Gemini 系）のいずれか。あらかじめ各 CLI でログインしてください |
| tmux | macOS であると便利 | ターミナルのセッション（作業状態）を保持するツール。入っていると **tako を再起動しても実行中のプロセスと画面が丸ごと復元**されるほか、**スマホからのリモート接続（`tako remote`）とオーケストレーターの worker 管理にはこれが必須**です。`brew install tmux` で導入。Windows 版は tmux を使わない方式に移行中のため、現時点では対象外です |
| [Tailscale](https://tailscale.com/) | リモート接続に必須（macOS） | `tako remote`（スマホからの接続）の transport。Mac とスマホの両方にアプリを入れて同一アカウントでログインすると、tailnet 内限定の固定 URL で安全に接続できます |
| git | あると便利 | git パネル（ブランチ・コミットグラフ・diff 表示）で使います。macOS では `xcode-select --install` で入っていることが多く、Windows では [Git for Windows](https://gitforwindows.org/) を導入します |
...
:::caution[Windows 版はテスター向けプレビューです]
Windows 対応は現在進行中です。ターミナルの基本機能（タブ・ペイン分割・日本語入力・`tako` CLI）、オーケストレーション（`tako master`）、git パネル、ファイルプレビュー（コード・Markdown・画像・PDF・動画）と Web ビューは使えます。一方で**セッションの完全復元**、**スマホからのリモート接続**、**アプリ内更新**はまだ対応していない、または挙動が限定されます。PDF はページ送り・ズーム・パンまで動きますが、**テキスト選択・PDF 内リンク・しおり**は Windows の PDF レンダラに API が無いため使えません。動画は OS の再生エンジンを使うため、**再生できる形式は Windows が対応しているコーデック**（H.264 など）に限られます。

機能ごとの対応状況は[**Windows 対応状況**](/windows-support/)にまとめてあります。手元の環境での状態は、インストール後に次のコマンドでも確認できます。

```powershell
tako platform
```

不具合を見つけたら [GitHub Issues](https://github.com/takushio2525/tako/issues) までご報告ください。
:::
...
お使いの OS のタブを選んでください。このページの他のタブ切り替えも、選んだ OS に連動します。
...
<p>
<a href="https://github.com/takushio2525/tako/releases/latest" class="tako-btn tako-btn-primary">GitHub Releases から最新版をダウンロード →</a>
</p>

macOS 版の zip（`tako-vX.X.X-macos-arm64.zip`）をダウンロードし、以下を実行します。
...
Windows 版は GitHub Releases で配布しています。**インストーラー**（推奨）と、展開するだけで使える**ポータブル ZIP** の 2 種類があります。
...
1. 上のボタンから最新リリースを開き、**`tako-setup-vX.X.X-x64.exe`** をダウンロードします
2. ダウンロードした `.exe` をダブルクリックして実行します。管理者権限は不要で、**現在のユーザー向け**にインストールされます
3. 「WindowsによってPCが保護されました」という青い画面が出たら、下の「SmartScreen の警告が出たら」を参照してください
4. 画面の指示に従って進めます。「**tako を PATH に追加する（推奨）**」は既定で有効になっているので、そのまま進めれば `tako` コマンドがどこからでも使えるようになります
5. インストール後、**新しく** PowerShell を開いて確認します

```powershell
tako --version
```

バージョン番号（例: `tako 0.5.12`）が表示されれば成功です。

:::note[「新しく開く」のが大事です]
PATH の変更は、すでに開いているターミナルには反映されません。`tako` コマンドが見つからないときは、PowerShell を一度閉じてから開き直してください。
:::

### ポータブル ZIP で入れる

インストーラーを使わず、展開したフォルダから直接起動する方法です。USB メモリに入れて持ち運ぶ場合や、インストーラーを実行できない環境で使います。

<details>
<summary>ポータブル ZIP の手順（PATH は手動で通します）</summary>

1. 最新リリースから **`tako-vX.X.X-windows-x64.zip`** をダウンロードします
2. 好きな場所に展開します（例: `%LOCALAPPDATA%\Programs\tako`）。フォルダの中には GUI 本体の `tako-app.exe` と、コマンドラインの `tako.exe` が入っています
3. `tako-app.exe` をダブルクリックすると tako が起動します
4. どのフォルダからでも `tako` コマンドを使いたい場合は、展開先を PATH に追加します

```powershell
# 展開先に合わせてパスを変えてください
$takoDir = "$env:LOCALAPPDATA\Programs\tako"
$userPath = [Environment]::GetEnvironmentVariable('Path', 'User')
[Environment]::SetEnvironmentVariable('Path', "$userPath;$takoDir", 'User')
```

PATH を追加したら、**新しく** PowerShell を開いて `tako --version` が通ることを確認してください。

</details>

### つまずきポイント: SmartScreen の警告

Windows 版はまだコード署名を行っていないため、初回実行時に **Microsoft Defender SmartScreen** の警告（「WindowsによってPCが保護されました」）が表示されます。次の手順で実行できます。

1. 警告画面の **「詳細情報」** をクリックします
2. 展開された下部に表示される **「実行」** ボタンをクリックします

ダウンロードした zip が「ブロック」扱いになっている場合は、zip を右クリック →「プロパティ」→ 下部の**「許可する」にチェック**を入れてから展開してください。

:::caution[信頼できる入手元から取得してください]
この警告は「配布元の身元が証明されていない」という意味で、必ずしも危険を示すものではありません。ただし回避操作を行う以上、**必ず上記の [GitHub Releases](https://github.com/takushio2525/tako/releases/latest) から取得**し、第三者が再配布したファイルは実行しないでください。
:::
...
スタートメニューから **tako** を検索して起動します（ポータブル ZIP の場合は展開先の `tako-app.exe` をダブルクリック）。既定のシェルとして PowerShell が 1 ペイン開きます。
...
5. **指示とテンプレートの準備** — 指示ファイルが未作成なら安全な開発ルールの既定値を作り、セットアップ用ファイル一式を tako のデータフォルダ（macOS は `~/Library/Application Support/tako/setup/`、Windows は `%APPDATA%\tako\setup\`）に展開します。既存の指示は上書きしません
...
Homebrew 経由なら `brew upgrade --cask takushio2525/tako/tako` でも更新できます。

</TabItem>
<TabItem label="Windows">

Windows 版のアプリ内更新は対応準備中です。当面は [GitHub Releases](https://github.com/takushio2525/tako/releases/latest) から最新のインストーラー（`tako-setup-vX.X.X-x64.exe`）をダウンロードし、そのまま実行して上書きインストールしてください。ポータブル ZIP の場合は、tako を終了してから展開先のファイルを新しい zip の内容で置き換えます。

</TabItem>
</Tabs>

各バージョンの変更内容は[リリースノート](/releases/)をご覧ください。
...
- **macOS**: Homebrew でインストールした場合はターミナルを開き直してみてください。ZIP からインストールした場合は上記の「PATH を通す」手順を確認してください
- **Windows**: PATH の変更は既存のウィンドウには反映されません。PowerShell を閉じて開き直してください。それでも見つからない場合、ポータブル ZIP なら「ポータブル ZIP で入れる」の PATH 追加手順を実行してください
...
**macOS**: quarantine 属性（ダウンロードしたアプリに付く隔離マーク）が原因のことがあります。解除してから再度起動してください:

```bash
xattr -dr com.apple.quarantine /Applications/tako.app
```

**Windows**: ダウンロードした zip が「ブロック」扱いのまま展開されていると、実行できないことがあります。zip を右クリック →「プロパティ」→ 下部の「許可する」にチェックを入れてから展開し直してください。

### 再起動したらタブが消えていた

**macOS**: tmux バックエンドが無効になっている可能性があります。`tako persist` で状態を確認し、`tako persist on` で有効化してください。tmux 自体が未インストールの場合は `brew install tmux` で導入すると、実行中プロセスごと完全復元されるようになります。

**Windows**: プレビュー版では、tako を終了すると実行中のプロセスは終了します。タブ・ペインの構成と作業ディレクトリは復元されますが、画面内容と実行中プロセスの完全復元は対応準備中です。
