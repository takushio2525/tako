...
Windows 対応は現在進行中です。ターミナルの基本機能（タブ・ペイン分割・日本語入力・`tako` CLI）、オーケストレーション（`tako master`）、git パネル、ファイルプレビュー（コード・Markdown・画像・PDF）と Web ビューは使えます。一方で**セッションの完全復元**、**スマホからのリモート接続**、**動画プレビュー**、**アプリ内更新**はまだ対応していない、または挙動が限定されます。PDF はページ送り・ズーム・パンまで動きますが、**テキスト選択・PDF 内リンク・しおり**は Windows の PDF レンダラに API が無いため使えません。
