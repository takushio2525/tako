...
| 状態 | 件数 | 意味 |
| --- | --- | --- |
| 対応 | 123 / 158（78%） | macOS と同じように使えます |
...
対応 28・一部対応 4・未対応 / 未実測 5

| 機能 | 状態 | 差分 | 根拠 |
| --- | --- | --- | --- |
| `tako_open_file` | 対応 | — | 実機セルフテスト: 項目 66 / 66b / 68b / 112 / 114 / 116（dispatch・tako open・direction・新しいタブ） |
| `tako_jump` | 対応 | — | 実機テスト: 履歴の積み方・畳み方・上限・閉じたペインの開き直しは OS 分岐を持たない純粋関数で、jump_history の単体と dispatch の jump（行指定の open で積み、戻る / 進むで着地する）が CI の Windows ジョブで緑 |
