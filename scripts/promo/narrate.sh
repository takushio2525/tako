...
# 声ごとに出力レベルが違う（実測: say のピーク中央値 -12.3dB に対し VOICEVOX は -4.0dB）ので、
# そろえないと後段でクリップする。ただし**ピークをそろえてもラウドネスは一致しない**
# （クレストファクタが違う。同じ -12.3dB で say -25.3 LUFS / ずんだもん -31.9 LUFS）ため、
# 音量の最終的なつじつま合わせは build-explainer.sh 側が担う: あちらはナレーションのバスを
# 測ってから目標ラウドネス（TAKO_PROMO_NARR_LUFS）へ当てるので、固定ゲインは持っていない。
set -euo pipefail
