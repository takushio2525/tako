...
# ビート名 → 秒（無ければ数値として解釈）
beat_time() {
    local scene=$1 anchor=$2 f="$SCENES_DIR/$scene-beats.tsv"
...
while IFS=$'\t' read -r id kind source anchor offset min_dur caption subtitle speech; do
    ...
    card)
        ...
        ffmpeg -v error -y -loop 1 -framerate "$FPS" -t "$dur" -i "$png" \
    ...
    clip)
        ...
        t=$(beat_time "$source" "$anchor") || exit 1
        ...
        if [ -n "$caption" ] || [ -n "$subtitle" ]; then
            ...
            ffmpeg -v error -y -ss "$start" -i "$src" -vf "$vf,format=yuv420p" \
...
ffmpeg -v error -y -f concat -safe 0 -i "$list" -c copy "$video"
...
ffmpeg -v error -y "${inputs[@]}" -filter_complex_script "$fc" -map "[narr]" -t "$VDUR" -ar 48000 -ac 2 "$narr_only"

if [ -f "$BGM" ]; then
    fade_start=$(/usr/bin/python3 -c "print(max(0.0, $VDUR - 3.0))")
    # BGM は薄く（-16dB 相当）。ナレーション中はさらに sidechain で下げる
    ffmpeg -v error -y -i "$video" -i "$narr_only" -stream_loop -1 -i "$BGM" \
        -filter_complex "[2:a]atrim=0:${VDUR},asetpts=PTS-STARTPTS,volume=0.16,afade=t=in:st=0:d=2,afade=t=out:st=${fade_start}:d=3[bgm];[bgm][1:a]sidechaincompress=threshold=0.015:ratio=8:attack=40:release=700:makeup=1[duck];[1:a][duck]amix=inputs=2:normalize=0:dropout_transition=0[a]" \
        -map 0:v -map "[a]" -c:v copy -c:a aac -b:a 192k -movflags +faststart -shortest "$OUT"
else
    echo "!! BGM が無い（$BGM）。ナレーションのみで書き出す" >&2
    ffmpeg -v error -y -i "$video" -i "$narr_only" -map 0:v -map 1:a -c:v copy -c:a aac -b:a 192k -movflags +faststart -shortest "$OUT"
fi
