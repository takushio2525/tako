...
make_real_claude_proxy() {
    ...
    printf '%s\n' \
        ...
        'if { [ "${1:-}" = "auth" ] && [ "${2:-}" = "status" ]; } ||' \
        '   { [ "${1:-}" = "mcp" ] && [ "${2:-}" = "list" ]; }; then' \
        '  exec /usr/bin/env HOME="$TAKO_REAL_HOME" PATH="$TAKO_REAL_PATH" "$TAKO_REAL_CLAUDE_BIN" "$@"' \
        'fi' \
        'printf '\''claude\n'\'' >"$HOME/unexpected-agent-launch"' \
        'exit 0' >"$path"
    chmod +x "$path"
...
run_real_setup() {
    ...
    env \
        ...
        PATH="$bin" \
        TAKO_ISOLATED=1 \
        TAKO_REAL_HOME="$TAKO_REAL_HOME" \
        TAKO_REAL_PATH="$TAKO_REAL_PATH" \
        TAKO_REAL_CLAUDE_BIN="$TAKO_REAL_CLAUDE_BIN" \
        "$TAKO_BIN" setup </dev/null >"$output" 2>&1
}
...
# 任意: 実ユーザー認証は読み取り専用、書き込み先だけスクラッチ HOME にして実測する。
if [ -n "${TAKO_REAL_CLAUDE_BIN:-}" ] &&
    [ -n "${TAKO_REAL_HOME:-}" ] &&
    [ -n "${TAKO_REAL_PATH:-}" ]; then
    REAL_HOME="$TMP/real-auth-home"
    REAL_BIN="$TMP/real-auth-bin"
    mkdir -p "$REAL_HOME" "$REAL_BIN"
    make_login_shell "$REAL_HOME/login-shell"
    make_real_claude_proxy "$REAL_BIN/claude"
    run_real_setup "$REAL_HOME" "$REAL_BIN" "$TMP/real-auth.out"
    assert_prompt_count "$TMP/real-auth.out" 0 "実 Claude 認証"
    assert_contains "$TMP/real-auth.out" '\[検出\] claude: .*認証済み / [a-z0-9-]+' "実 Claude の認証・プランを検出できない"
    assert_contains "$TMP/real-auth.out" '\[detected\] Claude プラン: [a-z0-9-]+' "実 Claude の検出採用ログがない"
    assert_contains "$TMP/real-auth.out" 'セットアップが完了しました' "実 Claude 認証ケースが完走しない"
    [ ! -e "$REAL_HOME/unexpected-agent-launch" ] || fail "実認証ケースが対話 agent を起動した"
    printf '[MEASURE] scenario=real-claude-auth before_keys=5+ after_keys=0 prompts=0 source=detected result=complete\n'
fi
