#!/usr/bin/env bash
# test-editor-keys-1652.sh — 修飾キー付きの打鍵の実 GUI 経路テスト（#1652）
#
# 隔離した data で **visual-test 版の実 tako-app** を立て、visual-test 節 `editor-keys` を
#   ① そのまま走らせる → 9 相（文書頭 / 語の左右 / 日本語の語 / ⇧ で選択 / 語の削除 /
#      smart Home / 桁の記憶 / Page Down / 文書末 / IME の変換中）がすべて緑
...
if grep -q "TAKO_VISUAL_TEST_OK" "$TMP/new.log"; then
  pass "9 相すべて緑（TAKO_VISUAL_TEST_OK）"
else
  fail "節が緑にならない"
