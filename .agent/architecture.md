...
- **ホバー・アニメーションは自動で追従する**: hover / active / tooltip の状態変化は
  ...
  再描画のループが自走する
- **`AnyView::cached` は入れ子にできない**（#801 の実測）: GPUI はキャッシュビューを
  実際に描き直すあいだ `window.refreshing = true` を立て、再利用の条件に
  `!window.refreshing` が入っている。したがって**キャッシュビューの中のキャッシュビューは
  一度も当たらない**。ペインヘッダを `PaneBody` の内側でさらにキャッシュしても効かない
...
ウェルカムバナー #549 1.17M（**初回起動時のみ表示**）/ ペインヘッダ 0.62M（上記の
「`cached` は入れ子にできない」で塞がっている）/ スナップショット残り 0.65M /
クローム 4 枚の cached 再利用 0.46M / ルートの箱 0.41M / gpui のフレーム下限 0.16M。
