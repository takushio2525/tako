...
- **拡張キーは tmux から外側端末へ伝わらない**: 内側が kitty protocol を要求しても
  外側 Term の DISAMBIGUATE は立たない → UI 層は**バックエンドペインで修飾付きキー
  （Shift+Enter 等）の CSI u 送出を常時有効化**する（`CsiUMode::ModifiedOnly`）。
  ...
  （`CsiUMode::Full` = 直接 spawn 構成）のみ。往復 + 「27u」非漏出は e2e 済み
