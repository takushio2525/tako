...
- **UI**: `TakoApp::preview_edits` がペイン別バッファを保持し、`preview_render.rs` は既存の
  選択 bounds / texts をカーソル・選択へ再利用する。編集時は入力ごとの syntect 全再解析を
  避けて平文を即描画し、タイトルバーに編集切替・dirty・保存結果を表示。IME は既存
