...
- **close 整合**: 明示 close（×・cmd+W・CLI / MCP close）= バックエンドセッションも kill。
  アプリ終了・クラッシュ = detach のみ（= 永続化）。外部 tmux に attach しただけの
...
- **共存のための conf**（`<data_dir>/tmux-backend.conf`、毎起動再生成）:
  `status off` / `prefix None`（tmux の UI・キー介入ゼロ）、`mouse on`（ホイール =
  tmux copy-mode スクロール。自前スクロールバックの代替。スクロールバー・`tako scroll` は
  alt screen 扱いで無効化される = 既知のトレードオフ）、`allow-passthrough on`、
...
- **マウスレポートの保証**（tako の存在意義。Zed の同症状が自作の動機）:
  ...
  copy-mode = チャットが遡れる
- **拡張キーは tmux から外側端末へ伝わらない**: 内側が kitty protocol を要求しても
  外側 Term の DISAMBIGUATE は立たない → UI 層は**バックエンドペインで CSI u 送出を
  常時有効化**する（tmux が extended-keys always + `extended-keys-format csi-u` で
  内側の要求形式へ再エンコードするため安全。Esc の往復も e2e 済み）
