...
- **プレビュー描画キャッシュ**（`preview_render::PreviewImageCache`）: PDF ページ画像・
  画像・動画サムネの `Arc<gpui::Image>` と PDF テキストレイヤの `Arc<Vec<PdfTextLine>>` を
  pane ごとに保持し、path 不変の間は Arc clone のみで描画（ensure は render 冒頭で
  path 比較 1 回）。無効化は set_preview / ペイン・タブ close
