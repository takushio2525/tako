...
- 項目 116（#835）の `file URL が 4 本ともパスへ戻る (1)` は**両側の POSIX 前提**が原因
  （器とも #906 の符号化とも無関係）: テスト側 `self_test::file_url` が `path.display()` を
  そのまま符号化して Windows で `file://C%3A%5CUsers%5C…` を作り、製品側
  `open_files::file_url_to_path` は復号結果が `/` 始まりでないと `None` を返していた。
  4 本のうち通るのは `/` 始まりのダミー 1 本だけ = 観測値と一致
- 決め手は**同じ RFC 8089 の規則が `osc_tap`（OSC 7 の cwd 追従）に既にあり、そちらだけ
  Windows 形を扱えていた**こと = #870 / #873 と同型。`tako_core::file_uri` を新設して
  `strip_drive_slash` を移し、`osc_tap` と `open_files` の両方が通る形へ。
  **プラットフォームで分岐しない**（判定は URI の形だけなので macOS から Windows 形を検査できる）
- `percent_decode` の 2 実装は**統合しない**（不正入力の方針が用途で違う。OSC 7 は `%zz` を
  拒否 / 開く経路は素通り）。理由を `file_uri` の doc に明記した
- 実機の before/after: main（修正なし・#906 セッションで実測）= **項目 116 で FAILED** /
  このブランチ = **項目 116 通過**（`tabs=3->6` で macOS と同じ 3 タブ）。117 / 118 も通り
  **到達範囲 0〜118**。同一バイナリの A/B（`TAKO_913_LEGACY=1`）は**単体で決定的**に取った
  （Issue 記載の壊れた形 `file://C%3A%5CUsers%5C…` が再現して 2 本 FAILED）。実機の legacy
  アームは項目 116 の手前で 3 回フレークし、**直後の after アームも同じく早期で落ちた**ので
  環境ドリフトと切り分けた
- 検証: macOS `TAKO_APP_SELF_TEST_OK` 完走（skip 3 = 蓋閉じの既知。項目 116 も従来どおり緑）/
  test --workspace 2484 passed 0 failed / fmt / clippy(-D warnings) /
  クロスチェックの**警告リストが main と完全一致** / 実機テストはベースライン一致
- 番犬テスト（ドライブ判定を `open_files` で書き直したら落ちる）を追加。走査は
  **テストモジュールより前だけ**にする（全体だと番犬自身の文字列に当たって自分で落ちる）
- 次の壁は **#920 へ起票**（項目 119 = #868。セルフテストが unix の導入手順をリテラル期待）
- 作法: 1 回目の run は項目 111 が孤児（`-L tako-iso-*` 4 個）で落ち、掃除後に同じバイナリで通った。
  **作業は専用 worktree**（共有ツリー `~/dev/tako` は main のまま）
## 2026-08-23（#915 + #854 + #917: handoff をプロジェクト単位へ再設計し旧形式を自動移行）
- 置き場を `handoff/projects/<key>.md`（管轄する master の後任にだけ渡る）と
  `handoff/<profile>.md`（プロファイル運用メモ = 共通置き場・常に渡る）へ分離。管轄は
  明示引数 → プロファイル担当 + 同タブの稼働 worker → worker のみ の順（**明示引数を
  最優先にしたのは Issue の列挙順と違う**: worker が 1 体でもいれば推測が常に勝ち引数が死ぬ）。
  決まらなければ本文を貼らず一覧とパスだけを渡す。運用メモ 80 行超で警告（肥大の再発防止）
- 旧形式は **setup 実行時 + master が読む経路**の二段構えで必ず自動移行（#916 の第一適用例）。
  割り方はトップレベル `##` 単位で、持ち主は マーカー → 見出しのキー → 見出しの語とキーの
  先頭要素の一致。**本文からキーを探さない**（本番 default.md 528 行で `tako` 等が
  6 プロジェクト誤ヒットして材料にならなかった実測）。冪等 / 原本を `handoff/archive/` へ
  退避（失敗したら移行しない）/ 実施を可視化 / 持ち主不明の断片は運用メモへ残す
- **冪等は 1 回踏んだ**: 番地 `<!-- tako:handoff-format: projects -->` を入れる前は、担当
  1 件のプロファイルで 2 回目の setup が運用メモの雛形ごと projects/ へ吸い込んだ（隔離 e2e で実測）
- #854: プロファイルを caller_role と**ペインの role ラベル**の両方から解決（非既定優先）。
  インライン前置きはシェルへ export されないので claude を撃ち直した master は env を失う
  （報告の `TAKO_ORCHESTRATOR_ROLE=[master]` と一致）。`profile_source` で出どころが見える
- #917: 後任は退役 master のペインを分割して作る = 旧ペイン close で分割前の矩形を継ぐ
...
- 合流: #915（PR #922）が先に main へ入ったので merge し、引き継ぎの移行も登録簿から
  発火する形へ一般化（分割移行は Step で表せないので手順は handoff_store のまま、**発火と
  可視化だけ**を機構が引き受ける）。setup の発火点も 1 本へ統合。「何が起きたか」は専用実装の
  フラグではなく**中身の前後比較**で決める（`MigrationOutcome::migrated` は「プロジェクトへ
  移した行があるか」なので、形式マーカー付与だけのファイルで status と run が食い違った）
- 自省: 隔離せず `ensure_migrated()` を呼ぶテストを書いて**本番へ空の .lock を約 170 本**
  作った（うち 158 本は停止済みインスタンスの残骸ぶん）。原因は 2 つで、①テストの隔離漏れ
  ②`migrate_one` が対象ファイル全部に無条件でロックを取っていた（`config_io` のロックは
...
- GPUI の `AnimationElement` は**終わっていないフレームで毎回** `request_animation_frame()` を
  呼ぶので、`repeat()` を `CommandState::Running` に紐づけると**エージェントのタブでは永久に
  フレームを要求し続ける**（#786 / #801 / #803 で削った毎フレームの固定費が復活する）。
  推奨案 2 を「oneshot で有限回（2 秒 × 3）」の形で実装。role で切らず**時間で切った**のは、
  `npm run dev` のようなエージェント以外の長寿命プロセスが role では漏れるのと、
  毎フレームの時刻比較を残さず**止まることを GPUI の構造に保証させる**ため
- 実測（隔離 GUI・1 タブ 1 ペイン・`sleep 600` = **出力ゼロ**・同一バイナリ A/B。
  `TAKO_945_LEGACY=1` が旧挙動）: tako-app 自身の CPU が **19.09% → 2.93%**。
  ペイン 1 枚でこの値なので、原因が出力ではなく脈動だったことも同時に確定した
- **判定を「描かれたフレーム数」に置かない**のが要点: 蓋閉じ（`Display is turned off` →
  `Clamshell Sleep`）だとディスプレイリンクが止まり両アームとも 0 になる（CPU 実測も蓋が
  開いていた時間帯でしか取れなかった）。セルフテスト項目 128 は**アニメーターが計算した
  不透明度**を見る（時間を空けて描き直しても動かない = `done` = 要求も止まっている）
- 検証: 隔離セルフテスト `TAKO_APP_SELF_TEST_OK`（項目 128 新設。`early spread 0.644` /
  `late spread 0.000` / `restart opacity 0.592`）+ `TAKO_945_LEGACY=1` で項目 128 が FAILED
...
- ペインの右クリックに「このペインでリモート接続…」を追加（**そのペイン自体が SSH になる** =
  ペインもタブも増えず pane ID も不変。素のシェルへ ssh の 1 行を #640 の送達確認つき経路で
  打つので失敗すればプロンプトへ戻る）+ 開き先の既定を新タブ →「いまのタブへ新ペイン」へ。
  語彙の正本は `tako_core::remote_open`（`split` / `tab` / `pane`）で CLI `--target` / MCP
  `target` / GUI の分岐が同じ表を引く。判定 `can_ssh_pane` も 1 本なので**メニューの出し分けと
  dispatch のガードが同じ答えを返す**（出た項目が断られない）
- **踏んだ罠**: 器（tmux）つきペインの**外側** alt screen を判定へ渡すと、tmux クライアント
  自身が alt screen なので **persist 有効（= 既定）の全ペインで項目が出ない**。#694 が
  `pane_inner_alt_screen` で回避していた同型。隔離セルフテストで実測（`outer_alt=true` /
  `inner_alt=false`）→ 番犬 `remote_open_watchdog` で再発を止めた
- **併せて直した**: GUI のファイルメニュー経路が dispatch を通っておらず（独自に `ssh` を
  ペインのプログラムとして spawn）**#919 の契約が GUI だけ効いていなかった**（到達不能ホストで
  タブごと消える）/ `drive_command_flows` にペイン単位の直列化（同一ペインへ 2 本積むため）
- 検証: 品質ゲート全緑 + クロスチェック エラー 0・警告 10（全件未変更ファイル由来）+
  隔離セルフテスト `TAKO_APP_SELF_TEST_OK`（項目 131 新設）+ **実 SSH 先での通し**
  （既定 = 同じタブにペイン +1 / target=pane = pane ID 不変で同じペインがリモートのプロンプトへ /
  到達不能ホストはシェルへ戻る / 実行中ペインは理由つき拒否 / MCP 141 ツールで同一操作）
- 関連コミット: `2fbb7ba`（PR #1017）。**メニュー描画のスクショは未取得**（画面 OFF で
  `screencapture` が失敗）= 目視はユーザー確認待ち。Windows 実機は offline で未検証
## 2026-08-28（#1011: `claude agents --json` の起動コストを削る — #1001 C1）
- 走査先ごとに Node を並列起動する形（#571 の対策として正しい設計）が、実測 4 走査先で
  1 スキャン **CPU 0.89 秒 / Node 4 本**。master 稼働時は TTL 5 秒が実効間隔になり 1 コアの約 18%
- **①前段ガード**: claude の台帳 `<config dir>/sessions/<pid>.json` を直読みして
  「その走査先に live な claude が居るか」を Node 無しで判定し、居ない**明示アカウント**の
  起動を省く。**答えは従来どおり `claude agents --json`**（台帳は「起こさなくてよい Node」の
  見分けにしか使わない = 誤っても Node を起こす側へ倒れる）
- **②鮮度の用途分離**: `AgentScanFreshness`（Monitoring 5s = master の worker 監視 /
  Ui 30s = 画面表示・一覧・定期スキャン）。**キャッシュは 1 本のまま**にしたのが肝で、
  用途ごとに分けると同じ瞬間に 2 本走って Node 起動が**倍**になる（削るはずのものが増える）
- **実測（同一バイナリ A/B・`TAKO_1011_LEGACY=1` が旧挙動・交互 3 反復）**:
  Node **4 → 2 本** / CPU **0.89 → 0.46 秒（−48%）**。鮮度分離だけを見る列
  （監視 → 6 秒 → UI → 監視）は **8 本 / 1.80 秒 → 4 本 / 0.95 秒**。
  **結果は両アームで完全一致**（`session_id` 順の安定ダンプを diff して 0 行差）
- **壊していないことの確認**: #571 e2e（汚染 `CLAUDE_CONFIG_DIR` 下で実 claude の
  busy → idle を watch が検知）が両アームで緑（13.6s / 13.7s）/ #877 e2e が両アームで
  同一結果（13 件・同一 `status`）/ `cargo test --workspace` 全緑
- **自己検証（上流のレイアウト変更対策）**: 起こした走査先について「CLI が台帳の知らない pid を
  返していないか」を毎回突き合わせ、取りこぼしを見つけたら**プロセス寿命でガードを止める**。
  既定 config dir は `always_scan` で常に起こすので、レイアウトが変われば**次のスキャンで**検出される。
  検出力は故障注入 `TAKO_1011_INJECT_LEDGER_GAP=1` で実測（1 回目 2 本 + 信頼=false →
  2 回目 4 本 0 省き = 旧挙動へ自動復帰）
- **調査で確定したこと（レポート §7 C1 の 4 項目への回答）**: ①走査先の統合は**不可**
  （`claude agents --help` に複数 config dir を見る手段が無い）②③実装した
  ④**台帳は CLI を完全に置き換えられる**（フィールド一致・`status` 逐次更新・終了で消える）
  = Node 起動 0 本にできるが、公開仕様でない内部レイアウトなので今回は昇格させず判断材料を残した
...
- `TextBuffer::find_all` が**小文字化した写しのバイト位置を元テキストの位置として使い**、
  さらに終端に**元クエリ**のバイト長を足していた（二重にずれる）。`İ` U+0130 は 2 → 3 バイト、
  `ẞ` U+1E9E は 3 → 2 バイト。実測では位置がずれるだけでなく**アプリごと落ちる**経路もあった
  （全置換で `not a char boundary` panic / 終端が本文の範囲を超える out of bounds panic）
- 直し方は**探索は写しに対する高速な部分文字列探索のまま残し、位置だけを元テキストへ戻す**
  （`Lowered` = 写し + ずれた文字だけの記録。ASCII / 日本語では記録が空なので恒等）。
  文字単位の総当たり比較は**実測 1.9x 遅く最悪 72x** だったので採らなかった。採用案は
  実ファイル 2.9MB で**旧より速く（0.56〜0.64x）最悪ケースも 1.86x** に収まる
- **本文全体のバイト長の一致は健全性の根拠にならない**（`İ` と `ẞ` が両方あると伸縮が
  打ち消しあって総和だけ一致し、途中の位置は食い違う。`İ needle ẞ` で実測）。
  クエリ側も 1 文字単位の小文字化に揃えた（`str::to_lowercase` は語末 Σ が文脈依存なので、
  両側を丸ごと小文字化すると**本文に在る部分文字列をそれ自身で検索しても見つからない**）
...
- 41 を通した先の壁は **#967**（項目 97 / 99 が画面の `tako setup` リテラルを期待。#898 で
  ...
  期待値を `welcome::launch_command_line` から作り折り返しに強い形へ。**両モードで完走**
