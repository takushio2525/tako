...
## 2026-08-05（main の Markdown / コードプレビュー改善を cherry-pick）

- ユーザー要望（08-01）を受けて #656（GFM テーブル + 配色・タイポグラフィ）/ #669（ライトテーマの
  構文色）/ #680（リンクの ⌘Ctrl+クリック + コードブロックのコピーボタン。130 ツール）を移植。
  #666 の土台は持ち込まず `pending_clipboard` だけ最小移植し、対応マトリクスは #521/#693 済みの
  実態へ直した（doc 再生成で base から溜まっていた 3 行のずれも解消）
- 検証: CI 3/3 緑 + 隔離実機（リンク 6 件の openable 判定 / 非 http 3 種の拒否 / コピーの
  クリップボード完全一致 / 範囲外拒否 / dark・light 8 枚の目視）。証拠は `~/dev/tako-evidence/743/`
- 関連: PR #743。main 改善 84 コミットの仕分け一覧は #588 のコメントに投稿
- 次: 仕分け B の #546（貼り付け）→ #608（support テストのフレーク）→ #684（PTY 行数）
