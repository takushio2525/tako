...
## 2026-08-24（#642: IME 未確定文字列オーバーレイの行高を明示してベースラインずれを根治）
- win467 の `edd1a55` を main の現状（#737 でオーバーレイが `ime_preedit_text` を
  チャットと共有する形になっている）へ合わせて移植。GPUI の `StyledText` は**行ボックスの
  高さとフォントサイズを継承側（`window.text_style()`）から取る**ので、`.h()` だけでは
  行ボックスが既定の `phi()`（1.618 × 13px ≒ 21px）のままになり、17px の箱に対して
  `(21-17)/2 ≒ 2 論理px` 下へずれる（125% で物理 2.5px = 行高の約 12%）
- 箱・文字・行ボックス・TextStyle を**IME 対象ペイン**の値へ揃えた（アンカーは元から
  `cell_size_for_pane` 由来のペイン単位なので、theme のグローバル値だと Ctrl+= した
  ペインで行単位までずれる）。`ime_preedit_text_styled` を切り出してチャット側と 1 実装を共有
- **チャットのインライン表示（#737）は触っていない**: そちらはミラー行
  （`terminal_screen_lines` の div）も同じ継承既定を使うので**相対ずれが無い**。
  片方だけ直すと逆にずれる（実装を読んで確認した）
- 検証: fmt / clippy(-D warnings) / `test --workspace` 緑 / クロスチェック エラー 0・警告 12
  （ベースライン同数）/ 新規テスト 4 本（DPI ごとのずれ量を数値で固定。`pixel_snap` は
  `f32::round()` ではなく half-toward-zero なので 150% の 25.5 は 25 へ落ちる）。
  番犬の検出力は 2 通りの revert（`.line_height()` 削除 / `theme` へ戻す）で FAILED を実測
- **実機未検証**（Windows 実機 offline）: 変換中プレビューのベースライン目視。#937 へ
