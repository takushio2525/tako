...
- `autorename` の起動を `spawn_claude(bin, prompt, strict)` 1 本へ寄せ、`--strict-mcp-config`（`--mcp-config` 無し = MCP を一切使わない）を付ける。**再試行の引き金は非ゼロ終了だけ**で、フラグ無しの再試行が通ったときにだけ「使えない」とプロセス内で学習する（打ち切り・起動失敗では再試行しない = 上限のぶん二重に待たせない）
- macOS 実測（claude 2.1.258 / MCP 9 本 / 順序交互 10 ラウンド。順序固定の初回 15 ラウンドは **2 番目の腕が系統的に遅く**、偏りを見つけて測り直した）: 命名プロンプト p50 18.4→16.9s（対応差の中央値 -3.8s・9/10 で速い）/ 最小プロンプト 5.8→3.7s / **子孫プロセスのピーク 17→2**。隔離 GUI の 1 対 1 は 32.8s（`TAKO_758_LEGACY=1`）→ 16.6s で同じタブ名
- 単体 5 本（偽 CLI 3 + 学習規則 2）。フォールバックを外した実装で 3 本が FAILED = 検出力あり。`CLAUDE_TIMEOUT` は**縮めない**（フラグ付きでも最大 43.6s）
