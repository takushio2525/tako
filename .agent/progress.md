...
- **受け入れ検査 rework 1 回目**: CI の macOS ランナーは `/bin/bash`（3.2）なので `set -u` 下の空配列 `"${answers[@]}"` が `unbound variable` で落ちた（PASS=43 FAIL=9。手元の bash 5 では出ない）。`${arr[@]+"${arr[@]}"}` の慣用句へ直し、規約を `.agent/conventions.md` へ追加
- 実測: `scripts/test-setup-deps-prompt-1499.sh` **52 PASS 0 FAIL**（`/bin/bash` 3.2 / bash 5 の両方・10 秒・CI の macOS ジョブへ登録）。A/B `TAKO_1499_LEGACY=1` は端末でも聞かない = Issue の症状。番犬 5 本・注入 7 通りすべて FAILED → 戻して緑。workspace 5040 passed 0 failed・clippy 3 宇宙 0・fmt 0
