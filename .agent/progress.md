...
- `{{platform_notes}}` が対応マトリクスの縮退理由を**全文**並べており、Windows で `platform` 片が 4110 B・tako が作る側が取り分（18944 B）を 1.7〜2.7 KB 超過（CI 実測 20708 / 21420 / 21639 = そのぶん利用者の `prompt_blocks.append` の取り分が削られる）。#1154 / #1477 の作法どおり prompt には件数と引き方だけを残し、理由の全文は動的 topic `platform`（`tako orchestrator guide platform` / MCP `tako_orchestrator_guide`）へ回した。短縮形・手順書・A/B は `PlatformFacts` の 1 実装（`notes_section_in` / `full_section_in`）を共有する
- 予算の番犬が**実機でしか測れない**のが元凶だったので、`Platform` を prompt 組み立てまで引数で通し（`system_prompt_pieces_on` / `build_prompt_pieces_on` / `render_prompt_placeholders_on`）、`prompt_budget_1477` は macOS / Windows **両方の形**を measure する。#1278 が付けた `#[cfg_attr(windows, ignore)]` は外した
- 実測: platform 片 4110 → 412 B / base 20709→17118・21421→17830・21640→18049（同一バイナリで旧テンプレート + A/B から main を再現して比較）・A/B `TAKO_1571_LEGACY=1` で番犬 3 件が FAILED → 戻して緑・workspace 3179 passed 0 failed・clippy 3 宇宙 0・check-windows error 0・docs 32 ページ / og 31 / 内部リンク 1581 本 OK
