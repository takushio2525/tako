...
| `${ext}` | 拡張子（ドットなし・小文字） | `'c'` |
| `${workspaceRoot}` | プロジェクトのルート（#1656。無ければ git のルート → ファイルのディレクトリ） | `'/Users/a'`（`/Users/a/Makefile` がある場合） |
...
3. 拡張子既定マップのうち **settings.json のユーザー定義**
4. **プロジェクト既定**（#1656。`Cargo.toml` / `package.json` / `pyproject.toml` / `go.mod` /
   `*.csproj` / `Makefile` を上へ辿り、種別の既定をそのルートで走らせる。表は
   `tako_core::runner_project::KINDS`、辿る範囲は `tako_core::project_root`。詳細は FR-3.18）
5. 拡張子既定マップのうち**組み込み既定**（2 章）
6. どれも無ければ**エラー**（メッセージに宣言の書き方 1 行例と
...
| 拡張子なしファイル（Makefile 等） | 宣言があれば実行可。`Makefile` / `makefile` / `GNUmakefile` は #1656 のプロジェクト既定で `make`（`run` 規則があれば `make run`）。それ以外は実行不可（拡張子キーの特殊化はしない） |
