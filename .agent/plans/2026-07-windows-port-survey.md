...
> **この A/B/C は `.agent/plans/2026-07-windows-persistence-backend.md`（#518）で実コードに基づき
> 再評価済み。結論は C のままだが、上表の「機能制限」欄は実態より広く見積もられている**
> （送達確認は GUI 稼働中は失われない / スクロールは縮退しない / report は pane_log で代替可能）。
> 縮退の正確な範囲はそちらの §4 を見ること。
