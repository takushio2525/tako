...
1. **実機の before/after**（受け入れ条件 ①）。`TAKO_899_LEGACY=1` が before。
   道具は実機の `~\arm-after.ps1` / `arm-before.ps1` / `run-arms.ps1` / `kick.ps1` に置いてある
2. 実機テストのベースライン 22 件（#930 込み）の照合
3. **実機のリポジトリはブランチ `w899` のまま**。`main` へ戻す
4. 1〜2 が緑なら PR #931 を squash merge → #899 クローズ → この節の「⚠」を外す
