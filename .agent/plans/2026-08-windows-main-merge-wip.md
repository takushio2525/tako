...
14. **計測する側の DPI 認識を宣言しないと、健全なウインドウが「1.25 倍あふれている」ように
    見える**（#1063 で 1 本の Issue を丸ごと生んだ）。素の `powershell.exe` は **DPI 非認識**で、
    そこから測ると**片方だけが仮想化される**:

    | 呼ぶもの | DPI 非認識のプロセスから見た値 |
    |---|---|
    | `GetWindowRect` / `GetClientRect` / `SetCursorPos` / `SM_CXSCREEN` | **物理 ÷ 倍率**（125% なら 1938x1038 → 1550x830） |
    | `Graphics.CopyFromScreen`（BitBlt） | **物理のまま** |

    だから「ウインドウの寸法」で切ったスクリーンショットは**縮小ではなくクロップ**になり、
    右 1/5 と下 1/5 が画像から消える（実測: crop 仮説 99.87% 一致 / downscale 仮説 75.42%）。
    クリックも仮想化されるので、非認識の座標で「表示」を押すと「ウインドウ」が開く。
    **`SetProcessDPIAware()` はシステム DPI 認識**（マルチモニタで倍率が違うと足りない）なので、
    `SetProcessDpiAwarenessContext((HANDLE)-4)` = PerMonitorV2 を宣言し、
    **宣言が効いたことを `GetAwarenessFromDpiAwarenessContext` で検算**してから測る。
    道具はリポジトリの `scripts/windows/measure-window.ps1`（宣言が効かなければ実行を拒否する）。
    tako 側の実測値は MCP `tako_check_health` の `dpi_awareness` からも読める（#1063）

15. **`.ps1` に非 ASCII のコメントを書かない**（作法 11 の続き。#1063 で踏んだ）。
    BOM 無しで置いた `.ps1` を Windows PowerShell 5.1 は CP932 で読むので、UTF-8 の
    日本語コメント行が**次の行ごと食う**。しかも構文エラーにならず、**次の行の代入が
    静かに消える**（`$lim = $bmp.Width - 13` が実行されず `$lim` が null になり、
    フィルタが全件を落として「該当なし」になった）。計測スクリプトは ASCII 限定にする
    （`windows_dpi_manifest_watchdog` が `scripts/windows/*.ps1` について機械検査する）
...
## #1063 の記録（2026-09-02。「レイアウトが 1.22 倍あふれる」は計測の錯覚だった）

### 結論

**製品側の欠陥は無かった。** Windows 11・表示スケール 125% の実機で、最大化した tako の
UI（タブバー右端の ⌘K / ベル / 表示モード / テーマ、メニュー行のウインドウ操作ボタン、
下部ステータスバー）は**すべて見えており、実クリックにも反応する**。

症状を作っていたのは #1058 の調査で使ったヘルパー `.ps1` が **DPI 非認識**だったこと。
素の `powershell.exe` は DPI 非認識なので、そこから測ると片方だけが仮想化される
（作法 14 の表）。同じ瞬間・同じウインドウに対する A/B:

| | DPI 非認識の計測（#1058 / #1063 が使ったもの） | PerMonitorV2 の計測 |
|---|---|---|
| `GetWindowRect`（最大化） | **1550x830** ← Issue に載っている数字 | **1938x1038** |
| `GetClientRect` | 1536x816 | **1920x1020** |
| `CopyFromScreen` の出力 | **1550x830 = 物理ウインドウのクロップ** | 全体 |
| 「表示」の位置（x=165）をクリック | **「ウインドウ」が開く** | **「表示」が開く** |
| 右端 4 ボタン / ステータスバー | 画像から消える | 見える |

クロップであることは数値でも確定した: 非認識の画像は物理画像の左上を `dx=2, dy=2` で
切り出したものと **99.87% 画素一致**、1/1.25 の縮小仮説は 75.42% しか一致しない。

Issue の「198 桁 x 9.525px = 1886px がウインドウ 1550px を 336px 超える」も、
**1886 は物理・1550 は仮想化された値**という取り違え。物理で揃えると
`1886 / 1920 = 0.982` で **34px 余っている**（ペインの余白ぶん）。セル幅 9.5253px 自体は
Issue の実測どおりで、間違っていたのは比較対象だけだった。
`__COMPAT_LAYER=DPIUNAWARE` で `cols` が 198 のまま変わらなかったのも整合する
（tako 側は倍率 1.0・論理 1536px で組み直すので桁数は変わらない。計測の錯覚も変わらない）。

### 実機の実測値（`ssh win` / Windows 11 / 1920x1080 / 125%）

```
screen                      1920 x 1080     DPI 120（= 125%）
tako window awareness       2（per-monitor）        <- マニフェストは効いている
GetDpiForWindow(tako)       120
maximized client (physical) 1920 x 1020
  => logical (÷1.25)        1536 x 816
ruler: X + 98 spaces + X    span 943px / 99 cells => cell 9.5253 physical px (7.6202 logical)
cols=198 x 9.5253           1886 px  <= client 1920 px（0.982 = 収まっている）
```

実クリック（PerMonitorV2 の driver・window 相対の物理座標）:

| 対象 | 座標 | 結果 |
|---|---|---|
| テーマ切替（右端） | (1895, 72) | `tako theme` が dark → **light** |
| ステータスバー Files | (57, 1009) | `tako panel` の filetree false → **true** |
| 表示モード切替 | (1852, 72) | 変化なし ← **#1058 の穴 A の陽性対照**（この実機バイナリは修正前 main） |

最後の 1 行が効いている: **同じ driver・同じ行の 43px 隣のボタンは反応する**ので、
「クリックが届いていない」のではなく「そのボタンだけが `.occlude()` を欠いていた」
= #1058 の診断が正しかったことの独立な裏づけになる。ついでに
**#1058 が「#1063 のせいで物理的に押せない」として諦めた実クリック A/B は取れる**
（画面外ではなかったため）。

### なぜ Issue にせず放置しないか

前提（実行ファイルへ PerMonitorV2 のマニフェストが埋め込まれていること）は tako の
コードに 1 行も現れず、**gpui の既定フィーチャ `windows-manifest`** に乗っているだけ。
落ちても「ぼやけるだけで一見動く」し、macOS では原理的に再現しない。落ちたら #1063 が
**本物になる**ので、不変条件として明示して二段構えで見張ることにした（PR 参照）:

- `tako_core::platform::dpi`（境界 B24）で実測 + 起動時の申告 + MCP `tako_check_health` への公開
- セルフテスト項目 139（Windows 実機で毎回走る）
- 番犬 `windows_dpi_manifest_watchdog`（macOS でも走る = CI で早く落ちる）
- 計測の道具 `scripts/windows/measure-window.ps1`（宣言が効かなければ実行を拒否）

### 未実測（正直に）

- **表示スケール 100% のディスプレイでの確認**。owner の実機の表示設定を無断で
...
> **2026-09-09 更新 ①（#1114）**: `psmux_backend` は**間欠**だった。
> `器はクライアント切断後もattachで内容ごと戻る` が待ち不足で偽 FAILED になり、
...
#### #1127 の記録（セルフテスト項目 137 = #1040 の SSH 再接続。2026-09-04）

**結論: 項目 137 は根治し、実機の隔離 GUI セルフテストが初めて `TAKO_APP_SELF_TEST_OK` で
完走した**（skip 20 件はすべて理由つきの既知）。ただし完走には
**#1133 の回避（スタック拡張）が要る**ので、素のビルドでの完走はまだ未達。

##### 真因は製品側（`detect_disconnect` に #1090 の手当てが無かった）

#1102 で広げた **12 行の失敗診断**が 1 発で決めた。実機（44 桁のペイン）の画面:

```
testuser@host: Permission denied (publickey)   ← 理由（折り返し 1 行目）
.                                              ← 折り返しの尻尾
tako: … への接続に失敗しました（ssh           ← マーカー（折り返し 1 行目）
敗しました（ssh exit ）。理由は上の行です      ← マーカー 2 行目 = `ssh exit ` の最初の出現
```

`detect_disconnect` の枝 ① は「マーカーを含む最初の行の**直前の非空行**」を理由にするので
理由が **`.`** になる。`is_recoverable_reason(".")` は **true**（読めない理由は繋ぎ直す側へ
倒す規約）なので、**鍵の失敗でも打ち直して相手の認証ログを埋める** =
この関数が防ぐはずのものが起きる。`ssh_progress::classify` は #1090 で
「ssh 自身の失敗行を優先して拾う」手当てを済ませており、`is_ssh_error_line` の doc は
「#1040 の再接続判定が同じ表を引くために公開している」と書いてあるのに、
**枝 ① だけ取り残されていた**（枝 ② は使っていた）。

##### 途中で潰した別の穴（fixture 側）

fixture は本文を `print_lines` の**引数**で渡していたので、シェルがエコーした
コマンド行にも本文が載っていた。純関数へ直接入れて確認した機序:

| 画面 | `detect_disconnect` の理由 |
|---|---|
| エコー行あり | `Some("TAKO_1040_RETRY")` → 打ち直し |
| エコー行なし | `Some("testuser@host: Permission denied (publickey).")` → permanent |

実機では**エコー行の `ssh exit ` が折り返しで分断されていた**ので最初の出現にはならず、
これは主因ではなかった。それでも本文をファイル経由にする形へ寄せた
（`ShellDialect::print_file` を新設。数値マーカーで同じ罠を避けている `arith` の文字列版）。
**PowerShell 5.1 の `Get-Content` は既定でコードページ依存**なので `-Encoding UTF8` が要る
（実測: 付けないと `への接続に失敗しました` が `縺ｸ縺ｮ謗･邯壹↓…` に化ける）。

##### 実機 A/B（同じ機・同じレシピ・スタック拡張も同条件）

| arm | 結果 |
|---|---|
| before（`eba05b6`） | **項目 137 で FAILED**。12 行 tail に折り返しの `.` と 2 行に割れたマーカーがそのまま出る |
| after（`a8ac6a1`） | **`TAKO_APP_SELF_TEST_OK`（完走）** / skip 20 |

macOS: `test --workspace` 3383 passed 0 failed / fmt / clippy（両 feature）/
隔離セルフテスト `TAKO_APP_SELF_TEST_OK` / クロスチェックの警告集合が main と IDENTICAL。
実機 `cargo test --workspace --no-fail-fast`: **一意な失敗 16 件 = ベースラインと一致**
（この run は `psmux_backend` が全緑 = #1114 の入れ替わり flake は出なかった）。
検出力は修正を戻して `left: Some(".")` を実測。

##### 測り方: #1133 のスタックオーバーフローは `editbin` で迂回できる

項目 80（#380）で **main がスタックオーバーフローしてプロセスごと落ちる**（#1133）。
セルフテストを 137 まで進めるには回避が要るが、**項目 80 を env で飛ばすと項目 81 の
前提（生きたウィンドウハンドル）が壊れて別の場所で落ちる**ので使えない。
**ソースを触らずに PE ヘッダのスタックを広げるのが正解**:

```
& "…\VC\Tools\MSVC\<ver>\bin\Hostx64\x64\editbin.exe" /STACK:33554432 <path>\tako-app.exe
```
...
## #1114 の記録（psmux の永続化 e2e が間欠的に落ちる。2026-09-09）

**結論: 真因は待ち不足**（「器の起動競合」でも「attach の再接続タイミング」でもない）。
落ちるのは 1 か所（phase 1 = 器の中の pwsh へマーカーを打って出力を待つところ）で、
残り 2 か所（切断後の生存確認 / 再 attach の復元）は桁で余っていた。

### 測り方（フェーズごとの実測を採ってから直した）

再現を待つ前に、**現行コードのどの待ちに余裕が無いか**を測った
（`psmux_backend.rs` へ計測だけ足した版を実機で回す。数値は Windows 11 / 12 論理 CPU）:

| フェーズ | 予算（旧・固定） | 単独・アイドル | スイート並列（18 本） | 4 多重 + 負荷 12 |
|---|---|---|---|---|
| プロンプトが出るまで | 60 回 × 100ms ≒ **6 秒** | 1.0〜1.2s | **6.1〜7.1s** | **21〜34s** |
| マーカーが出るまで | 60 回 × 100ms ≒ **6 秒** | 0.4〜0.5s（6 回に 1 回は使い切り） | 0.6〜1.9s | 1.3〜20s |
| 切断が器へ届くまで | `sleep(800ms)` 固定 | ≦0.8s | ≦0.8s | ≦0.8s |
| `exists` が真になるまで | （待たない） | 0.04s | 0.05〜0.14s | 0.15〜0.43s |
| 再 attach で画面が戻るまで | 200 回 × 100ms ≒ **20 秒** | 0.10s | 0.10〜0.31s | 0.20〜1.27s |

**プロンプト待ちだけが 1 倍を割っている**。スイート並列（= このバイナリが自分で作る負荷）
だけで既に予算を超え、`rounds=2`（1 周目の窓を使い切った印）が 10/10 で出る。
テストが緑なのは**外側の 6 周リトライが吸収しているから**で、余裕はもう無い。

### 再現（`--workspace` が使えないので多重起動で作った）

`cargo test --workspace --no-fail-fast` は #1264 でコンパイルできないため、
**同じバイナリを 4 多重 + プロセス生成負荷 12** で回して同じ負荷帯を作った:

| 条件 | 対象テストの FAILED |
|---|---|
| 単独・アイドル ×10 / スイート ×10 / スイート + CPU 負荷 12 ×10 / churn 負荷 ×8 | **0 / 38** |
| 3 多重 + 負荷 8 ×12 | 0 / 12（ただし `rounds` が上限 6 のうち **5** に到達） |
| **4 多重 + 負荷 12 ×16** | **9 / 16**（全件 phase 1 = `1 回目の器でマーカーが出力される`） |

落ちた回の画面は `"PS C:\…\Temp> Write-O"` ＝ **打鍵は届いていて、エコーの途中で
予算が尽きた**形。器の起動失敗でも attach 競合でもないことがここで確定する。

### 直し方

固定回数の窓を全部やめ、**期限 + `tako_core::wait_budget::state_wait_budget`**（伸ばすだけ・
4 倍で打ち切り）へ寄せた。`machine_busy()` は Windows でも読めるようにした
（`GetSystemTimes` の差分。`sysload` と同じ測り方 = **Windows で `load=unknown` にならない**）。
...
### 実機 A/B（**同一ラウンドで交互に走らせて**機の状態の差を打ち消す）

`before.exe` = `origin/main` のテスト / `after.exe` = 修正版。4 多重 + 負荷 12 churn。
...
**`TAKO_1114_LEGACY=1` は同一バイナリ内で phase 1 だけを旧の固定窓へ戻す**が、
兄弟の待ちが新方式のままだとスイート全体が軽くなって旧アームでも落ちない
（実測 0/16）。**旧の再現には修正前バイナリが要る**ので、A/B は 2 本並べて測ること。

### 番犬

`crates/tako-control/tests/psmux_e2e_wait_watchdog.rs` が
`psmux_backend.rs` の①`for _ in 0..N {` の本体に `sleep` がある形 ②`sleep` の直後 3 行以内に
`assert` が来る形、を名指しで落とす（A/B の旧アーム `legacy_*` だけ除外）。
修正前のファイルへ当てると **9 か所を行番号つきで FAILED**（131 / 181 / 562 / 821 / 826 /
873 / 974 / 1051 / 1159）。

## #1271 の記録（psmux の後始末が返らず器がリークする。2026-09-09）

**結論: 「期限が無い」だけでなく「聞いても見つからない残骸がある」の二段**。後始末に
期限を付けるのは必要条件で、それだけでは `__warm__` サーバーが残る。

### 残骸の数え方（**次の実機作業はここから始める**）

psmux のパッケージは `psmux.exe` / `tmux.exe` / `pmux.exe` の 3 つを同じ実体で配る
（実測）。**サーバープロセスは、起動したクライアントの実行ファイル名を名乗る**ので、

- テストの器（`psmux_bin()` = PATH の `psmux`）→ `psmux.exe server -s … -L …`
- 製品経路（`tmux_backend` は `tmux` として起動する）→ `tmux.exe server -s … -L …`

の両方が出る。`Get-Process psmux` だけで数えると**片方が丸ごと 0 に見える**
（#1114 の「走る前の残骸は無かった」はこの穴）。数えるときは必ず 3 つ:

```powershell
foreach ($n in @('tmux','psmux','pwsh')) { "{0}={1}" -f $n, @(Get-Process -Name $n -ErrorAction SilentlyContinue).Count }
```

**自分の実行由来だけを取り出す**には、器のコマンドラインに載る隔離ソケット名で絞る
（`Get-Process` の名前一致で **kill してはいけない**。他ワーカーの器を巻き込む）:

```powershell
Get-CimInstance Win32_Process -Filter "Name='tmux.exe' OR Name='psmux.exe'" |
  Where-Object { $_.CommandLine -like '*tako-m2test-*' -or $_.CommandLine -like '*tako-si-test-*' }
```

この機の開始時点の残骸は **`tmux=24 psmux=1 pwsh=61`** で、24 件はすべて他セッション由来
（`-L tako-w1133` = 隔離 GUI の器）。以下の表はこの 24 を引いた「自分由来」で読む。

### psmux の実測（設計を決めた材料）

| 問い | 実測 |
|---|---|
| 器が居ないソケットへ `display-message -p '#{pid}'` は器を起こすか | **起こさない**（rc=1 `no server running`・器の数は不変） |
| 生きた器の pid は聞けるか | 聞ける（`display-message -p '#{pid}'` が返す） |
| 1 ソケットの器は何個か | **2 個**。本体 + `__warm__`（**本体の子**。`server -s __warm__ -L <socket> -x 120 -y 30`） |
| `taskkill /PID <本体> /T /F` は `__warm__` も落とすか | **落とす**（子・孫まで SUCCESS） |
| 本体だけ落とす（`/T` 無し）と `__warm__` は | **生き残る**。しかも `display-message` は rc=1 「器は居ない」と答える |
| その残骸を消せるのは | **`kill-server` だけ**（rc=0 で消える） |
| `wait-for <chan>` は返らないか | **即座に rc=0 で返る**（注入には使えない） |
| `run-shell "<長いコマンド>"`（`-b` 無し）は | **返らない**（注入に使える。器が無ければ起こしてから塞ぐ） |

**後始末の時点では本体が既に退役していることが多い**（テストがセッションを閉じた後）。
注入アームの診断行では 10 ソケット中 9 個が `server_pid=None` で、
残ったのは全部 `__warm__` だった。だから後始末は
「pid を先に聞く → 期限つき `kill-server` → 生きていれば pid で止める →
**素直に済まなかったら `kill-server` でもう一度掃く**」の 4 段になっている。

### 実機 A/B（Windows 11 / 12 論理 CPU / debug）

**① 連続実行で残骸が増えないこと**（`psmux_backend` を 12 回・修正版）

| i | 所要 | tmux | psmux | pwsh | 自分由来 | 結果 |
|---|---|---|---|---|---|---|
| 0（開始） | — | 24 | 1 | 61 | **0** | — |
| 1〜12 | 19.6〜26.0s | 24 | 1 | 61 | **0**（全回） | 18 passed × 12 |

修正前バイナリ（`origin/main` のテストソース）でも 12 回とも完走し `自分由来=0` だった。
**自然発生のハングは今日は再現しなかった**（#1114 の採取は半日 150 回規模）ので、
返らない状況は注入で作った。

**② 「後始末が返らない」の注入**（`TAKO_1271_INJECT=hang` = `kill-server` の代わりに
返らない `run-shell` を呼ぶ。器は生きたまま残る）

| arm | 完走したか | 自分由来の器 | pwsh |
|---|---|---|---|
| 旧（`TAKO_1271_LEGACY=1` = 素の `Command::output()`） | **しない**（180s で打ち切り・`test result` 行なし） | **+10 / 回**（0 → 10 → 20） | 62 → 81 → 100 |
| 新（期限つき + pid 指定 + 掃き掃除） | **する**（18 passed・47〜54s） | **0**（3 回とも） | 62（不変） |

**③ 余計な待ちが増えていないこと**（同一ラウンドで交互に 6 回）

| arm | 所要（6 回） | 平均 |
|---|---|---|
| before（`origin/main`） | 30.7 / 30.9 / 30.8 / 30.7 / 31.7 / 31.0s | **31.1s** |
| after（このブランチ） | 21.2 / 23.0 / 23.4 / 22.4 / 23.1 / 22.0s | **22.5s** |

**全ラウンドで after が速い**。後始末そのものは pid を聞くぶんだけ増える
（1 ソケットあたり **146ms → 229ms**・10 ソケットの実測）が、`raw()` 側で
出力の採り方をパイプから一時ファイルへ替えたぶんが上回る。切り分けは
「後始末だけ旧経路へ戻す」アーム（`TAKO_1271_LEGACY=1` 単独）で 21.6s = 新と同じ
＝ **8.6s の差は後始末ではなく `raw()` 群にある**。

### 番犬

`crates/tako-control/tests/psmux_cleanup_timeout_watchdog.rs` が
`psmux_ctl.rs` / `psmux_backend.rs` / `shell_integration_powershell.rs` の 3 ファイルから
`.output()` / `.status()` / `.wait_with_output()` と「殺していない子の `.wait()`」を
名指しで落とす（`legacy_` 接頭辞の A/B 旧アームだけ除外）。
修正前のファイルへ当てると **8 か所を行番号つきで FAILED**
（psmux_backend 36 / 74 / 107 / 533 / 577、shell_integration_powershell 449 / 597 / 714）。

### 残る限界（正直に）

- **器を一度も聞けないまま `kill-server` も返らない**回は、pid が分からないので止められない。
  掃き掃除の `kill-server` が最後の手段で、それも返らなければ諦めて `leaked=true` を出す
  （コマンドライン走査で pid を探す手はあるが、`Win32_Process` の列挙は 1 回が重く、
  素直に片付く経路まで遅くする）
