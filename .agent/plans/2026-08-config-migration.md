...
## テキスト置換で表せない移行（#915 との合流）

引き継ぎのプロジェクト単位化（#915 / PR #922）は**1 ファイル → 複数ファイルの分割**なので、
`Step`（テキスト → テキスト）では表せない。手順は `orchestrator::handoff_store` が持ち、
`migrations::run` が呼んで結果を `MigrationReport` の語彙へ翻訳する（`handoff_reports`）。

**番地に載せる価値は「どこで直るか」を 1 本にすること**にある。載せないと
`tako migrate` から見えず、発火点が分かれる（実際、合流直前は `tako setup` に
「handoff だけを直接呼ぶ経路」と「全種別を回す経路」の 2 つがあった）。

### 専用実装のフラグを信じない

`MigrationOutcome::migrated` は「プロジェクトへ移した行があるか」という**実装寄りの意味**で、
形式マーカーの付与だけで済んだファイルは false になる。それをそのまま報告したら
`status` の予告（2 件）と `run` の報告（1 件）が食い違った。**中身を前後で比較**して
「変わったか」を決める形に直した。委譲するときは、委譲先のフラグの意味論に
報告の正しさを預けないこと。
