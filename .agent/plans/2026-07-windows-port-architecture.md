...
| B5 | プロセス検査・制御 | 検査: `platform::procinfo`（`all_pids` / `parent_of` / `name_of` / `descendants_of` / `listening_ports_of_pid` / `image_path` **［実行中プロセスの実行ファイルパス。#936］**）／制御: `platform::process::terminate` **［制御側は P0 で新設済み］** | `tako-core/src/ports.rs`、`tako-control/src/agents.rs:81`、`sleep_guard.rs:829`、`remote.rs`（signal 送出）、`stale_binary.rs`（`pidpath`） | libproc（`proc_pidpath` 含む） / `kill(2)` | Toolhelp32 + `GetExtendedTcpTable` + `QueryFullProcessImageNameW` / `TerminateProcess` |
...
| B8 | OS 連携（開く・ゴミ箱） | `platform::os_integration`（**［#617 で Windows 実装済み］** `reveal` = `explorer /select,` / `open_default` / `open_with` / `open_with_dialog` / `open_url`（**［#1371］** `cmd /C start` から移した）= `ShellExecuteW` / `move_to_trash` = `SHFileOperationW` + `FOF_ALLOWUNDO` / `file_manager()` = 表記の出し分け。`notify` は Windows 未実装で `false` を返す） | `dispatch.rs:1691,6246-`、`sidebar.rs:1438,1450`、`main.rs:568-575` ほか `open` 16 / `osascript` 6 箇所 | `open` / AppleScript | `ShellExecuteW` / `SHFileOperation` |
