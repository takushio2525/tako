...
| B19 | ペインの文字コード | `platform::console::pin_pane_to_utf8_when_ready()` / `pin_needed()` **［#655 で新設・#659 で器の内側へ拡張］** | `tako-core/src/terminal.rs` の `TerminalSession::spawn`（PTY 直下の子）+ `backend::wrap_spawn_for_pane`（**器の内側**のシェル。`SessionBackend::pane_pids` 経由） | 恒等（no-op。`LC_CTYPE=UTF-8` の既定注入で担保済み） | 疑似コンソールを `AttachConsole` + `SetConsoleCP` / `SetConsoleOutputCP` で UTF-8 へ固定（ConPTY の既定は OEM コードページ = 日本語版 Windows で CP932。放置すると子の UTF-8 出力を conhost が CP932 と解釈し、**tako が受け取る前に**壊れる）。**器（psmux）を使うペインでは PTY 直下の子はクライアントでありシェルではない**ので、器へ `#{pane_pid}` を尋ねて器の内側にも同じ固定を当てる（#659） |
