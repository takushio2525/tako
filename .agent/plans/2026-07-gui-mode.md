...
> G4 実装（2026-08-02 / #739）。仕様から動かした点と、その理由:
>
> - **カードを「本体 + ▾」の 2 領域に割った**（スプリットボタン）。当初案の「カードの中へ
>   ▾ を入れ子で置く」だと、押下がカード本体の起動と ▾ の開閉のどちらになるかが
>   GPUI のイベント伝播に依存する。領域を**兄弟**にすれば構造で決まるので、
>   「▾ を押したのに既定で起動した」という最悪の誤爆が起きえない
> - **▾ は選択肢が 2 つ以上のときだけ出す**（既定しか無いときは出さない）。Code Runner と
>   同方針で、押しても 1 件しか出ないシェブロンは初心者にはノイズなため
> - 一覧の中身は `starter::load_starter_profiles()`（background）。**既定は
>   `default.yaml` が無くても必ず先頭**に置く（`tako master` は引数なしで既定を作って
>   起動するので、選択肢としては常に存在する）。各行には担当プロジェクト →
>   起動フォルダ → モデルの順で最初に取れた手がかりを 1 行だけ添える
> - **プロファイル名は `orchestrator::validate_profile_name` を通ったものだけ**を
>   一覧にも起動コマンドにも通す。これはシェルへ書き込む文字列で、プロファイル
>   ディレクトリのファイル名は誰でも作れる（`$(...).yaml` を置ける）ため。
>   コマンド組み立ては `starter::starter_subcommand` の 1 関数に集約し、
>   最簡形（既定は引数なし）の規則は CLI の案内文と同じ
>   `orchestrator::launch_command` を共有する
> - 読み直しは**新しいポーリングを作らず** 2 秒 tick に相乗り。しかも
>   `starter_profiles_stale()` が「GUI モード **かつ** スターターが実際に出ている
>   **かつ** TTL（15 秒）切れ」のときしか true を返さないので、通常の
>   ターミナル利用中はディスクを触らない（#340 の常駐ポーリング棚卸しの方針）
> - ドロップダウンはルート直下のオーバーレイ（#361 / #453 と同じ）。ビューポート実寸で
>   幅・左端を詰め、下に入らなければ**上へ反転**し、上限高さ（300px）を超えたら
>   中でスクロールする（`starter_profile_scroll`）。狭幅・多件数でも画面外へ伸ばさない
...
> **G4 実装済み（2026-08-02 / #739）。これでエピック #691 の全フェーズが揃った**。
> 実装場所と判断:
>
> - プロファイル選択 ▾: `starter.rs`（`StarterProfile` / `load_starter_profiles` /
>   `starter_subcommand` / `render_starter_profile_menu_overlay`）+ 押下の合流点は
>   `TakoApp::starter_action_with_profile`。詳細な設計判断は §2.2 の G4 注記
> - ctx ヒント: `ChatPaneState::ctx_hint()` を**警告色と同じ根拠**（`ctx_gauge` の warn）
>   から導く 1 メソッドにした。「赤いのに逃げ道が出ない」「ヒントだけ出て色が変わらない」の
>   ズレが構造的に起きない。ヒントは文字ではなく**押せるボタン**で、押下は G3 の
>   スラッシュボタンと同一経路（`chat_slash_action(Compact)`）= 新しい書き経路を作らない
> - マトリクス（#515）: `tako_ui_mode` は `WIN_TERMINAL`（#517）のまま、
>   `tako_chat_copy` は新設した `WIN_GUI_CHAT`（#519）へ移した。**チャットだけは
>   永続バックエンドにも依存する**（G2 の帰結: チャット化されるのはバックエンドを
>   持つペインだけ）ので、縮退理由を実態に合わせた
> - i18n: 新規文言はすべて `ui_text/ui_mode.rs` の `tr!` カタログ + 日英検査テストへ登録。
>   一覧に出るプロファイル名・プロジェクト名は**ユーザーのデータなので訳さない**
> - docs: 機能紹介に `features/gui-mode.md` を新設し、CLI リファレンスへ
>   `tako ui-mode` / `tako chat copy` / `tako show-command` / `tako preview-copy-code`、
>   MCP 一覧へ対応 4 ツールを追加。**MCP 一覧はスナップショットと機械照合して
>   133 件ちょうど**に揃えた（#620 の 128 件から 5 件ぶんドリフトしていた）
> - 検証: セルフテスト項目 99（▾ の出現条件 / 一覧の中身 / 選択 → 送達 /
>   選択後とペイン消滅での後始末 / ctx ヒントの境界）+ visual-test 2 節
>   （「プロファイル ▾」= 10 件・狭幅・スクロール・dark / light、
>   「ctx ヒント」= ヘッダの赤画素が増える）+ 単体 5 本。
>   ▾ は**合成マウスで実際に押す**（#725 と同じ `PlatformInput`。座標は
>   `starter_chevron_bounds` = 描き終わった実矩形）ので、リスナーの配線が
>   外れていたら落ちる — 関数直呼びだけだと「処理は動く」までしか押さえられない
