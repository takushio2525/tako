...
6 PR 横断レビュー（2026-07-03）で得た運用知見。master system prompt に組み込み済み
（`default_system_prompt.md` の `quality-ops` ブロック）。プロファイル固有のモデル
振り分け（`delegate_guidance`）とは独立で、全 master に常に適用される。

1. **根因先行**: バグ修正を worker に投げる前に根因（再現手順・スタック・実測値）を
   master 側で揃えて Issue に書く。根因付き指示は worker の成功率を大幅に上げる
2. **同一ファイルの直列化**: 同じファイルを触るタスクを並行 worker に出さない。
   並行が不可避なら先行 PR の受け入れ条件を Issue に列挙し、マージ前に diff で生存確認。
   実例: PR #45 が PR #43 の経路を退行させた
3. **機械検証がない領域の DoD**: cargo test / セルフテストが効かない領域（Web フロント・
   実機 UI・IME・描画）は DoD に「操作ログまたはスクリーンショット」を必須化。
   実例: ctrl トグルが無機能のままマージされた
4. **統合レビュー層**: 複数 PR がまとまって入った後はレビュー専任 worker で横断レビュー。
   単体 PR の品質が高くても統合点の退行は通り抜ける
5. **Closes 判断は master が持つ**: 曖昧な報告の Issue は報告環境での再現・解消確認なしに
   worker にクローズさせない
6. **完遂の定義**: worker への指示は push → PR → squash merge → worktree 片付けまで
   を完遂条件として明示する。コミット止まりの取りこぼし実例あり
