...
- **#1370 は完了**（IPC 由来のレイアウト変更のあと 1 フレーム強制描画。隠れた窓・最小化・
  仮想ディスプレイでは notify だけではフレームが来ないので cols/rows が古いまま残っていた。
  判定は `protocol::changes_layout` の 1 実装・A/B は `TAKO_1370_LEGACY=1`）。**install 要**
