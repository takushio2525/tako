...
- **#913（項目 116 = #835 Finder の「このアプリケーションで開く」）を解消**（PR #921）。
  原因は**両側の POSIX 前提**（器とも符号化とも無関係）: テスト側 `self_test::file_url` が
  `path.display()` をそのまま符号化して `file://C%3A%5C…` を作り、製品側
  `open_files::file_url_to_path` は `/` 始まりでないと弾いていた。決め手は**同じ
  RFC 8089 の規則が `osc_tap` に既にあり、そちらだけ Windows 形を扱えていた**こと
  → `tako_core::file_uri`（新設）へ一本化。**プラットフォームで分岐しない**（判定は
  URI の形だけ）。A/B は `TAKO_913_LEGACY=1`
- **#906（項目 101 = #749 の自動ハンドオフ）を解消**。層は **psmux の `new-session` そのもの**
  （`アクセスが拒否されました。(os error 5)` / exit 5）。条件は **`-EncodedCommand` の base64 が
  `==` で終わること**で、**同一長で padding だけを変えると反転する**（448 / 544 / 576 の 3 点）。
  直し方は符号化の出口（`platform::shell::container_safe_script`）で二重パディングを作らない。
  A/B は `TAKO_906_NO_PAD=1`
- **解消済み（詳細は plan の各記録節と progress.md）**: #907（器つきペインへの非 ASCII 送達 →
  ...
  #877 / #875 / #873
- **A/B の env（同一バイナリで旧挙動へ戻せる）**: `TAKO_913_LEGACY` / `TAKO_906_NO_PAD` /
  `TAKO_907_NO_INJECT` / `TAKO_903_LEGACY` / `TAKO_866_KEEP_EXACT_TARGET`

## 実機セルフテストの到達範囲（#913 後の実測）

**到達範囲は項目 0〜118**（#913 で 116 が開き 117 / 118 も通った）。
`TAKO_913_LEGACY=1`（旧経路）に戻すと同じバイナリで項目 116 が FAILED になる。

次の壁は **#920**（項目 119 = #868 install_plan）。セルフテストが unix の導入手順を
リテラル期待している（`claude.ai/install.sh` / `.local/bin/claude`）が、Windows の計画は
`install.ps1` で区切りが `\` = **116 と同種の POSIX 前提**。製品側
（`agent_install::recipe(Windows, Claude)`）は正しいので、期待値を計画そのものから
作る形（#905 と同じ流儀）が筋。
...
- **#920（項目 119 = #868）**: 直せば 119 以降（#853 / #858 …）が開く。
  期待値を `agent_install::recipe(Platform::current(), Claude)` から作ればリテラルの
  二重管理が消える
...
- `.agent/plans/2026-08-windows-main-merge-wip.md`（「#913 の記録」節に file URI の
