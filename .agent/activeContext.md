...
- **#757 は完了**（ログイン失効を `api_error` から分離して `login_expired` / `relogin` +
  対象アカウント併記へ。判定順序は「ダイアログ > 失効 > 上限」。**install 要**）
- **#1273 は完了**（背景シェル / Monitor が残る入力待ちを idle と判定。真因は #289 の帰属ミスで、
  claude の生 status が busy だった = tako の補正には入っていなかった。**install 済み**）。
  横展開の調査は #1277（codex / agy も同じ申告を出すか）
- 残るフレーク #995 / #771 / #1122 を #1153 と同じ形（状態待ち + 番犬）へ寄せる。
  ...
  **#1265 / #962 も完了**（PTY 枯渇 / 2 秒アサート → 機構の観測値。番犬は `src/` も走査）。
  **#1274 も完了**（真因は「相対比較なら安全」という前提。言語グローバルはロック外で
  2 回読むと日英が混ざる。ui_text 17 本 + ui_text 外 3 本を `for_each_lang` の区間へ入れ、
  番犬 `ui_text::lang_watchdog` が修正前 17 本を名指し。legacy 40/40 → 修正後 0/40）
