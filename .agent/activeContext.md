...
- **棚卸しで起票した製品バグ（未着手）**: **#933**（ゴミ箱が完全削除 = データを失う）/
  **#934**（AI タブ命名が Windows で一度も走らない）/ **#935**（受け入れゲートが `sh -c`）/
  **#936**（古い claude の警告が出ない）
...
- **#933**（ゴミ箱が完全削除）が最優先。**データを失う差**でしかも UI の文言は macOS と同じ
- **#934 / #935 / #936**: どれも「境界へ寄せる」既存の型がある（#877 = B21 / #875 = B1 /
