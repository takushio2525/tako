...
## 現在の対象（2026-08-04 朝・GUI モードエピック #691 の G1〜G4 全フェーズ merge 完了）

- **エピック #691（GUI ライク表示モード）は計画フェーズ全完了・本番反映済み**（08-04 08:33 に
  GUI 再起動、7 タブ/12 ペイン喪失ゼロ）。クローズはユーザーの実使用確認待ち
- 08-02 完了分の続き: #739 G4 = スターターのプロファイル選択 ▾（合成マウスで実クリック検証）+
  ctx 80% 警告 + /compact ヒント + docs + manual-checks（PR #740 = 723c59f）
- 次の一手: ユーザーの実使用フィードバック待ち → 問題なければ #691 クローズ。
  残キューは「未着手・持ち越し」節を参照

---

## 以下は 07-30〜08-02 の経緯（新しい順ではなく時系列）
