...
## 現在の対象（2026-07-07・#95 + #100 マージ済み、実機反映待ち）

main = `4c62786`（#95 Enter 空振り修正 = PR #101、#100 品質パイプライン = PR #102 の両方入り）。
`build-app.sh --install` で /Applications へ配置済み — **反映にはユーザーの tako 再起動（⌘Q）が必要**。
...
- [ ] #95 GUI 経路の実機確認（tako 再起動後、busy な claude ペインへの Enter 再送）
