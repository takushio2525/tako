...
## 文字列をバイト位置で切らない（Issue #1728 / #1746 / #1756 / #1757）

文字列の添字（`&s[..N]` / `&s[a..]`）と `s.len()` は**バイト**で数える。UTF-8 では日本語が
1 文字 3 バイト・絵文字が 4 バイトなので、文字を数えたつもりの位置は多バイト文字の途中に
当たりうる。添字が途中に当たると `byte index N is not a char boundary` で **panic** する
（描画中なら GUI ごと落ち、CLI なら exit 101）。**ASCII だけで試すと必ず緑になる**ので、
気づくのは日本語のファイル名や出力を渡されたとき。同じ型が続いた:

- #1728: プレビューのツールチップと実行メニューが `&cmd[..60]` / `&plan.command[..40]`
  （日本語のファイル名で描画中に落ちた）
- #1746: `tako task gate show` が証拠を `&ev[..120]` で切った（日本語の出力で exit 101）
- #1756: テーマ色の検査が `hex.len() != 6`（**バイト長の比較**）のあと `&hex[0..2]` で切った。
  「赤色」は 3 バイト × 2 = 6 バイトなので長さの検査を通り、切るところで落ちた
- #1757: peer 送達の確認が控えたファイル長で `&raw[len..]` と追記分を切っていた
  （書き直されたファイルでは文字の途中を指す）。チャットのモデル名の `label[..1]`
  （空なら範囲外・非 ASCII なら文字の途中）

書くときの決まり:

- **表示の切り詰めは `tako_core::text::truncate_chars(s, max)` の 1 本**を通す（`…` 込みで
  `max` 文字以内・数えるのは `char`・表示幅は見ない）。`chars().take(n)` + `…` を自前で
  書かない（#1757 で tako-app の `truncate` と context_budget の私有版を寄せた。
  transcript の 1 行要約 `summary_line` も数える部分はここを通す）
- **長さの比較も文字で数える**: 「6 桁か」「上限を超えたか」を `s.len()` で見ない。
  `s.chars().count()` か、1 文字ずつ検査しながら数える（#1756 の `parse_hex_color`）。
  `len()` を使うのは**バイトの予算**（IPC のサイズ・ファイルの大きさ）を見るときだけ
- 分けるなら位置を文字境界へ丸めてから: `s.floor_char_boundary(i)` → `split_at`
  （#1728 の検索欄）。**外から来た位置**（控えたファイル長・差し替えられたカーソル）は
  `s.get(i..)` で受け、`None`（範囲外か文字の途中）を「位置が無効」として扱う
  （#1757 の peer_messaging は縮んだときと同じく全文を見る）
- 先頭の 1 文字を変えるなら `chars()` で取る（#1757 の `capitalize_first`）
- ASCII と分かっている位置（直前の `starts_with('-')`・ASCII の区切り文字を `find` した
  位置・自分で作った 16 進文字列）は添字で切ってよい。番犬の視界にある場所では、同じ行か
  1 行上に `// 切り出し安全: <理由>` を書く

機械強制は 2 本。`crates/tako-control/tests/issue1728_byte_truncate_watchdog.rs` は
`preview_render.rs` の先頭からの範囲添字を**すべて**落とし、
`issue1746_cli_byte_slice_watchdog.rs` は tako-cli の範囲添字に**丸めか理由コメント**を
求める。範囲添字の検出は共有部品 `crates/tako-control/tests/common/range_index.rs` の
1 実装（#1757）なので、番犬を足すときはこれを `#[path]` で取り込む（検出器を写さない）。
repo 全体へはまだ広げていない。危ない型（下限か上限が数値リテラルそのもの・`.min(` を
含む・`len()` から引く）に絞っても、本番コードに理由も丸めも無いものが 50 件ある
（2026-09-27 の実測。多くは `Vec` のスライスか ASCII と分かっている位置）ので、
先に全件を仕分ける必要がある
