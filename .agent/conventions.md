...
- **到達手段が変わると成立しない検査は、前提を診断行に出して外れたら測らない**。
  ホイールの半行スクロール（#159）は `session.scroll_pixels` を通る**直接ペイン**
  でしか端数が載らず、器（tmux）つきのペインは `mirror_scroll_pane` →
  `backend_scroll_px` のミラー経路（capture ベース = 行単位・1 フレームでは立たない）
  へ入る。前提を見ずに測っていたため `TAKO_PERSIST=1` を付けた実行では
  `fract=0.000 shift=0 zero_cost=0.00`（フレーム完全同一）で必ず落ち、
  **以降の節が 1 つも走らなかった**（#943）。既定のレシピ（`TAKO_ISOLATED=1` 単独 =
  persist OFF）では走るので、skip で機能が永久に未検証になる形にはならない
...
### 蓋閉じの検証はディスプレイスリープを止めてから

蓋を閉じたままディスプレイが眠ると、**`NSScreen` には仮想ディスプレイが残っているのに
GPUI の `cx.displays()` が空**になる（#1141 で実測）。狙いは外れて既定の面へ落ちるので
起動は止まらないが、その run は「置き先の検証」にならない（`display_placement.available`
が 0 になっているかで見分けられる）。長い検証は走っているあいだだけ
`caffeinate -d -w <pid>` を当てる（ユーザーの省電力設定は変えない）。
