...
```bash
$ curl -s --resolve "shiozawamacpro-1.tail5ed162.ts.net:443:100.64.109.27" \
    "https://shiozawamacpro-1.tail5ed162.ts.net/test"
```
...
```json
{
  ...
  "method": "GET",
  "headers": {
    "Host": "shiozawamacpro-1.tail5ed162.ts.net",
    ...
    "X-Forwarded-For": "100.64.109.27",
    "X-Forwarded-Host": "shiozawamacpro-1.tail5ed162.ts.net",
...
- Let's Encrypt 証明書（CN=shiozawamacpro-1.tail5ed162.ts.net, 有効期限 90 日）
...
| `X-Forwarded-For` | `100.64.109.27` | 接続元 tailscale IP |
...
```bash
$ tailscale whois 100.64.109.27    # → ノード情報 + ユーザー情報を返す (exit 0)
...
https://shiozawamacpro-1.tail5ed162.ts.net
```

| 要素 | 値 | 変更条件 |
|---|---|---|
| hostname | `shiozawamacpro-1` | `tailscale set --hostname` で変更可。**変更しなければ固定** |
| tailnet-suffix | `tail5ed162` | 管理画面 General → Tailnet name で変更可。**変更しなければ固定** |
| TLD | `.ts.net` | 不変 |

### 証明書の詳細

```
subject: CN=shiozawamacpro-1.tail5ed162.ts.net
issuer: C=US, O=Let's Encrypt, CN=YE2
有効期間: 2026-07-16 〜 2026-10-14（90日）
SAN: DNS:shiozawamacpro-1.tail5ed162.ts.net
```
...
`dig shiozawamacpro-1.tail5ed162.ts.net` は NXDOMAIN を返す。
...
| ノード | IP | 状態 | 経緯 |
|---|---|---|---|
| `shiozawamacpro` | 100.99.33.116 | offline | userspace-networking モード（初回 PoC 用。`/tmp/tailscale-poc/state`） |
| `shiozawamacpro-1` | 100.64.109.27 | **online** | TUN モード（本テスト用。`/var/lib/tailscale`） |
...
offline の `shiozawamacpro` を削除し、`shiozawamacpro-1` を `shiozawamacpro` にリネーム
することを推奨。
