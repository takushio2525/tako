...
## main.rs 大規模リファクタリング（現行 UI 修正完了後に実施）

> **現行の UI 改善・機能追加タスクがすべて完了してから着手する。**
> main.rs が 13,000 行超に肥大化しており、並列作業時の編集競合・可読性低下・
> ビルド時間増大の原因になっている。Zed のように crate/モジュール分割する。

- [ ] **コンポーネント別モジュール分割**: main.rs から以下を独立ファイルに切り出す
      - `tab_bar.rs` — タブバーの描画・イベントハンドリング
      - `pane_header.rs` — ペインヘッダー（ロールバッジ・状態表示・ボタン群）
      - `status_bar.rs` — ステータスバー（ctx メーター・usage・トグル群）
      - `sidebar.rs` — 左サイドバー（ファイルツリー描画）
      - `right_panel.rs` — 右パネル（tmux ビュー・git ビュー・タブグループカード）
      - `preview_render.rs` — プレビューペインの描画
      - `drawer.rs` — たまり場ドロワー
      - `keybindings.rs` — キーバインド登録・ハンドラ
...
目安: 各ファイルは 1,000〜2,000 行以内。Zed の crate 分割（editor/terminal/workspace 等）を参考にするが、
まずは tako-app crate 内のモジュール分割から始める（crate 分割は必要に応じて後段で検討）。
