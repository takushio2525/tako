...
## パネル UI 刷新（FR-2.16.4〜2.16.7。**次の実装タスク**・2026-06-12 仕様化）

> 第2部としてユーザーから指示済みの実装タスク。仕様の正は `requirements.md` FR-2.16。

- [ ] 下部ステータスバー新設（Zed / VSCode 風）: 左にファイルツリートグル、
      右に tmux 管理・git 管理のトグルボタン。上部の「◧ panel」ボタンは廃止して集約。
      トグル状態の取得・操作は CLI / MCP からも（開発不変条件。ファイルツリーの
      CLI / MCP 経路は新設）
- [ ] パネル内部タブの 1 本化: 現 agents ビューを「tmux」へリネームし、空表示バグのある
      旧 tmuxview を削除して統合。タブごとの「タブ名ラベル付き四角枠」+ 枠内に全ペインの
      入れ子表示。各ペイン行右にゴミ箱 → kill 確認 → kill。行は折り返し / 省略（…）で
      見切れさせない。セッション列挙が正しく動くことを保証（旧 tmuxview の空表示バグ解消）
