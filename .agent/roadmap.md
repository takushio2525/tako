...
- [ ] タブ・ペインの AI 自動リネーム（FR-2.12。素材 + rename API を MCP 公開、判断は
      AI 層。デバウンス + 安価モデル、手動優先、OFF 設定、ヒューリスティックフォールバック。
      2026-06-12 要望。着手は報告済みバグ修正の後）
- [ ] tmux セッションの見える化タブ tmuxview（FR-2.13。右端固定タブ + 一覧 + 確認つき
      kill。データ取得層と表示を分離し、一覧・対応付け・kill は CLI / MCP からも可能。
      2026-06-12 要望。消し忘れ tmux の検知・提案機能の土台）
