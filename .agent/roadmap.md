...
## Phase 5.5: tmux バックエンド永続化（ユーザー承認済み方式）

> 全 PTY を tmux session 化し、tako 再起動でセッション（実行中プロセス・画面内容）を
> **完全復元**する（2026-06-12 登録。実行方式はユーザー承認済み）。
> v0.2 に置いていたセッション永続性（FR-5.1 = 構成のみ保存・シェル内容は復元しない）の
> 上位互換であり、FR-5 はこの方式で再設計する（要件の更新は着手時）。

- [ ] 設計スパイク: TerminalSession の spawn を `tmux new-session` 経由に差し替える方式の検証
      （既存の tmuxview / tty 突き合わせ・シェル統合・kitty protocol との共存を含む）
- [ ] 再起動時の再 attach とタブ / ペイン構成の対応付け復元
- [ ] tmux 不在環境では従来の直接 spawn へ無害に劣化（ゼロコンフィグ原則）
...
- [ ] MCP ゼロコンフィグオンボーディング（FR-2.14。MCP クライアント検出 + ワンクリック
      登録 + 診断表示 + instructions 品質整備。登録・診断は CLI / MCP からも。
      2026-06-12 要望、配布前に必須）
