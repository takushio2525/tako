...
## Phase 5: コンセプト② — ワークスペース機能 → ▶ 再開（2026-06-13。FR-3.2 / FR-3.3 完了）

> 2026-06-12 に Phase 5.5（tmux バックエンド永続化）を先行するため一時中断 →
> **2026-06-13 に FR-3.2 から再開**。技術選定（syntect / git CLI / pulldown-cmark）は
> 確定済み（`architecture.md`）。

- [x] 左サイドバー: ファイルツリー（FR-3.1 / FR-3.7。2026-06-12 完成 →
      **2026-06-13 に「タブ = ワークスペース」のマルチルート表示へ刷新**: タブ内全ペインの
      cwd をワークスペースフォルダとして並べる。エディタ風の見た目 = ルート見出し行 +
      開いているファイルのハイライト。`requirements.md` FR-3.1）
- [x] コードプレビュー + シンタックスハイライト（FR-3.2。2026-06-13 完成。syntect を
      `Highlighter` trait で抽象化・行番号付き。dispatch `OpenFile` + `tako open` +
      MCP `tako_open_file`（計 21 ツール）+ layout.json 永続化。
      実装メモは `requirements.md` FR-3.1〜3.3）
- [x] Markdown プレビュー（FR-3.3。2026-06-13 完成。pulldown-cmark。`.md` は既定で
      レンダリング表示、タイトルバーの目アイコントグルで code ⇔ markdown 切替、
      mode は CLI / MCP からも操作可）
- [x] プレビューペインでの軽い編集（FR-3.5。2026-07-12、#126）
...
- [x] Web ビューペイン（FR-3.8。✅ 2026-07-13、#155。wry = WKWebView の
      `build_as_child` ネイティブ統合。直接操作・dock 退避/復帰・永続化・
      CLI `tako web` + MCP `tako_web`。詳細は `architecture.md`「Web ビューペイン」節）
...
## たまり場（FR-2.15）→ ✅ 完了（2026-06-14）

> 「見た目はタブから消したいが処理は生かしたい」ターミナルのタブ外プール。
> ステータスバーの「⏏ 退避」ボタンから下部ドロワーを開閉し、退避中ターミナルの一覧を
> 表示・D&D で復帰・確認付き kill。× ボタンは kill ではなく退避に変更。
> CLI（`tako shelve` / `unshelve` / `shelved`）・MCP（4 ツール、計 27）からも操作可能。
> レイアウト永続化対応（再起動で復元）。
...
## main.rs 大規模リファクタリング → ✅ モジュール分割完了（2026-06-24）

> main.rs が 13,736 行に肥大化していたため、UI 描画メソッドを 7 モジュールへ分離。
> **13,736 → 8,359 行（39% 削減）**。機能変更なし、全テスト緑。

- [x] **コンポーネント別モジュール分割**: main.rs から以下を独立ファイルに切り出し
      - `keybindings.rs`（373行）— アクション定義 + キーバインド + キー入力変換 + テスト
      - `tab_bar.rs`（209行）— タブバーの描画
      - `status_bar.rs`（302行）— ステータスバー描画 + トグル
      - `drawer.rs`（372行）— たまり場ドロワー描画
      - `sidebar.rs`（834行）— ファイルツリー描画 + コンテキストメニュー + インライン編集
      - `right_panel.rs`（1,928行）— tmux ビュー + git ビュー + パネルコンテナ
      - `preview_render.rs`（1,347行）— プレビューペイン描画（コード/Markdown/ホバー/ピン）
      - `pane_header.rs` は対象メソッドが計 70 行と小さく保留
- [ ] **共通 UI ユーティリティの抽出**: テーマ参照ヘルパー、ホバースタイル、アイコン描画等
- [ ] **TakoApp 構造体の分割**: 状態管理をサブ構造体（TabBarState, SidebarState 等）に委譲
- [ ] **テストの追加**: 分割後の各モジュールに単体テスト
- [ ] **ビルド時間の検証**: 分割前後で incremental build 時間を計測・記録

main.rs の残り 8,359 行は TakoApp::new（初期化）、render_pane（ペイン本体描画）、
マウス/スクロール/D&D ハンドラ、ControlHost / EntityInputHandler 実装、
Render 実装、セルフテスト、自由関数群で構成。さらなる分割は別タスクで検討。
